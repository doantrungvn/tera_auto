 ...
 */
 package ${packageName?lower_case}.${functionMaster.functionMasterCode?lower_case}.${functionMethod.functionMethodCode?lower_case}.${functionMethod.functionMethodCode?lower_case}inputbean;

<#list functionMethodInput.singleList as item>
 ...
 * @version $Revision$
 */
public class ${functionMethodInput.methodInputCode?cap_first } implements Serializable {
	...
	<#if functionMethodInput.singleList?size gt 0>
		<@printSingleList functionMethodInput.singleList/>
	</#if>
}
...
<#macro printSingle item>
	<#assign itemCode = "${item.methodInputCode}">
	<#assign dataTypeName = "">
	<#assign newListObj = " ">
	<#assign newObj = " ">
		<#switch item.dataType>

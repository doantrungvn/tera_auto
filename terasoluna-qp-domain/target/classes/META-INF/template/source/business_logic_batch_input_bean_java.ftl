...
package ${package}.batch.${module.moduleCode?lower_case }.${businessLogic.businessLogicCode?lower_case};
...
<#list singleInputList as item>
	<#switch item.dataType>
		<#case 0 >
			<#if businessLogic.batchType?has_content>
import ${package}.batch.${businessLogic.businessLogicCode?lower_case }inputBean.${item.inputBeanCode?cap_first };
			</#if>
			<#break>
	</#switch>
</#list>
...
<#macro printSingle item>
	 ...
	 * ${item.inputBeanName!""}
	 */
	private ${dataTypeName} ${itemCode} ${newListObj} ${newObj};
	
	/**
     * Get ${item.inputBeanName!""}
     *
     * @return ${item.inputBeanName!""}
     */

	...
	<#elseif sqlDesignOutput.dataType == 0 && !(sqlDesignOutput.sqlDesignOutputParentId?has_content)>
	     <#if sqlDesignOutput.designType == 0>
	     	<#if !moduleCode?? || moduleCode == ''>
	     		<#assign object_path = "${projectCode?lower_case}.batch.model.common.${sqlDesignCompound.sqlDesign.sqlDesignCode?uncap_first}OutputBean">
			<#else>
				<#assign object_path = "${projectCode?lower_case}.batch.model.${moduleCode?lower_case}.${sqlDesignCompound.sqlDesign.sqlDesignCode?uncap_first}OutputBean">
			</#if>
        <#else>
        	<#if !moduleCode?? || moduleCode == ''>

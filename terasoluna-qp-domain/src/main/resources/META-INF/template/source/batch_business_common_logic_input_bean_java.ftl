...
<#list singleInputList as item>
	<#switch item.dataType>
		<#case 0 >
			<#if businessLogic.blogicType == 1>
				<#if businessLogic.customizeFlg>
import ${package}.batch.service.commoncustomize.${businessLogic.businessLogicCode?lower_case }inputbean.${item.inputBeanCode?cap_first };
				<#else>
import ${package}.batch.service.common.${businessLogic.businessLogicCode?lower_case }inputbean.${item.inputBeanCode?cap_first };
				</#if>
			</#if>
			<#break>
	</#switch>

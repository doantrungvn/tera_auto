/*
 * @(#)${functionMethodOutput.methodOutputCode?cap_first }.java
 * 
 * Copyright (c) 2016 NTTDATA Corporation.
 */
 package ${packageName?lower_case}.${functionMaster.functionMasterCode?lower_case}.${functionMethod.functionMethodCode?lower_case}.${functionMethod.functionMethodCode?lower_case}outputbean;

<#list functionMethodOutput.singleList as item>
 ...
 * @version $Revision$
 */
public class ${functionMethodOutput.methodOutputCode?cap_first } implements Serializable {
	...
	<#if functionMethodOutput.singleList?size gt 0>
		<@printSingleList functionMethodOutput.singleList/>
	</#if>
}
...
<#macro printSingle item>
	<#assign itemCode = "${item.methodOutputCode?uncap_first}">
	<#assign dataTypeName = "">
	<#assign newListObj = " ">
	<#assign newObj = " ">
		<#switch item.dataType>
			<#case 0 >
				<#assign dataTypeName = "${item.methodOutputCode?cap_first}">
				<#break>
			<#case 16>
				<#if item.commonObjDefiCode?has_content>

/*
 * @(#)${decision.decisionTbName?cap_first }InputBean.java
 * 
...
<#list singleInputList as item>
	<#if item.dataType == 0>
import ${package}.${place}${moduleCode?lower_case }.${decision.decisionTbCode?lower_case }inputbean.${item.decisionInputBeanCode?cap_first};
	</#if>
</#list>
</#if>
 ...
 * @version $Revision$
 */
public class ${decision.decisionTbName?cap_first }InputBean implements Serializable {

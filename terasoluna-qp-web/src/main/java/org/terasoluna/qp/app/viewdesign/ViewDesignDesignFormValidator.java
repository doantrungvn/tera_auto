 ...
 */
@Component
public class ViewDesignDesignFormValidator implements Validator {
	...
	public boolean supports(Class<?> clazz) {
		return (ViewDesignDesignForm.class).isAssignableFrom(clazz);
	}

	@Override
	public void validate(Object target, Errors errors) {
		ViewDesignDesignForm viewDesignDesignForm = (ViewDesignDesignForm) target;
		String sqlPattern = viewDesignDesignForm.getSqlDesignForm().getSqlPattern();
		String[] reservedWords = systemService.databaseReservedWords(SessionUtils.getCurrentDatabaseType());
		
		if (FunctionCommon.checkExists(reservedWords, viewDesignDesignForm.getSqlDesignForm().getSqlDesignCode())) {
		...
		if(DataTypeUtils.equals(SqlPattern.SELECT,sqlPattern)){
			this.validateFromForm(viewDesignDesignForm.getFromForm(), errors);
			this.validateWhereForm(viewDesignDesignForm.getWhereForm(), errors);
			this.validateGroupByForm(viewDesignDesignForm.getGroupByForm(),errors);
			this.validateHavingForm(viewDesignDesignForm.getHavingForm(), errors);
			this.validateOrderByForm(viewDesignDesignForm.getOrderByForm(),errors);
			this.validateSelectForm(viewDesignDesignForm.getSelectForm(), errors);
		}
		if(DataTypeUtils.equals(SqlPattern.INSERT,sqlPattern)){
			this.validateIntoForm(viewDesignDesignForm.getIntoForm(),errors);
			this.validateValueForm(viewDesignDesignForm.getValueForm(),errors);
		}
		...
		this.validateInputForm(viewDesignDesignForm.getInputForm(),errors);
		this.validateOutputForm(viewDesignDesignForm.getOutputForm(),errors);
	}

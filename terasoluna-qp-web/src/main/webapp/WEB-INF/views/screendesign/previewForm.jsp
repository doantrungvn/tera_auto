<tiles:insertDefinition name="layouts-popup">
    ...
    <tiles:putAttribute name="additionalHeading">
        ...
        <script type="text/javascript" src="${pageContext.request.contextPath}/resources/app/screendesign/javascript/common.js?r=khhjhg4"></script>
		...
		<style>
			...
			form .qp-table-list thead tr th{
				${projectStyle['panelListTh']}
			}
			
			form .qp-table-list-none-action{
				${projectStyle['panelListTable']}
			}
			
			form .qp-table-list-none-action thead tr th{
				${projectStyle['panelListTh']}
			}
			...
			form .result-date-time{
				${projectStyle['panelListTdDateTime']}
			}
			
			form .result-no-number{
				${projectStyle['panelListTdNoNumber']}
			}
			
			form .result-action-column{
			...
			form .qp-table-form tr th label{
				${projectStyle['panelTableFormTh']}
			}
			
			form .qp-table-form tr th{
				${projectStyle['panelTableFormTh']}
			}
			
			form .qp-table-form tr td{
																				...
																				<c:if test="${ colIndex == 0}">
																					<tr index="${rowIndex }" style="${screenArea.rowStyle}">
																						<c:if test="${(empty screenArea.fixedRow || screenArea.fixedRow != 1) && rowIndex == 0 }">
																							<th style="vertical-align: middle;${screenArea.headerStyle}" rowspan="${screenArea.tblHeaderRow }">
																								<span style="${screenItem.style}"><qp:message code="sc.sys.0004"></qp:message></span>
																							</th>
																						</c:if>
																				</c:if>
																				<c:if test="${screenDesignForm.screenGroups.containsKey(screenItem.groupItemId)}">
																				...
																				<c:set var="styleFirstCol" value="align-left" ></c:set>
																						<tr index="${rowIndex }" style="${screenArea.rowStyle}">
																							<c:if test="${(empty screenArea.fixedRow || screenArea.fixedRow != 1) && rowIndexTemp == 0}">
																								...
																								<td rowspan="${screenArea.tblComponentRow }" style="${screenArea.inputStyle}">
																									<span style="${screenItem.style}">1</span>
																								</td>
																							</c:if>
																					</c:if>
																				<c:if test="${not empty screenItem.groupItemId }">	<c:set var="isGroup" value="true" />
																						<c:choose><c:when test="${screenDesignForm.screenGroups.containsKey(screenItem.groupItemId) && not empty screenDesignForm.screenGroups[screenItem.groupItemId] }">
																								...
																								<c:set var="styleGroup" value="0" ></c:set>
																									...
																									<td class='${styleFirstCol } ${styleGroup } ${fieldFormat}'  index="${colIndex }" style="${styleFirstColumn} ${isDisable } ${screenArea.inputStyle}" ${colSpanProperties} ${rowSpanProperties} >
																								</c:if>
																								<c:choose>

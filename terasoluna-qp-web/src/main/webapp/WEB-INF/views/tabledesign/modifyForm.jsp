<tiles:insertDefinition name="layouts">
	...
	<tiles:putAttribute name="body">
		<script id="tmp_list_table-template" type="text/template">
			<tr>
				...
				<td>	
					...
					<input type="hidden" name="listTableDesignDetails[0].columnId">
				...
				<td>
							...
							<c:forEach var="item" items="${tableDesignForm.listGroupOfDatatype}" varStatus="status">
								<optgroup label="${item.label}">
									<c:forEach var="dataType" items="${item.listOfDataType}">
										<option primitiveId = "${dataType.primitiveId}" value="${dataType.basetypeId}" codelistCodeAutocomplete="${dataType.codelistCodeAutocomplete}" codelistDefaultAutocomplete="${dataType.codelistDefaultAutocomplete}" sqlCodeAutocomplete="${dataType.sqlCodeAutocomplete}" baseTypeGroup="${dataType.groupBaseTypeId}" datatypeFlg="${item.datatypeFlg}" maxValue="${dataType.maxValue}" minValue="${dataType.minValue}" validationRule="${dataType.validationRule}" length="${dataType.length}" precision="${dataType.precision}" mandatory="${dataType.mandatory}" remark="${dataType.remark}" defaultValue="${dataType.defaultValue}" supportOptionFlg="${dataType.supportOptionFlg}" userDefineValue="${dataType.userDefineValue}" operatorCode="${dataType.operatorCode}" datasourceType="${dataType.datasourceType}" datasourceId="${dataType.datasourceId}" constrainsType="${dataType.constrainsType}" style = "background-color: ${dataType.color}">${dataType.label}<option>
									</c:forEach>
								</optgroup>
							</c:forEach>
						</select>
					...
					<input type="hidden" name="listTableDesignDetails[0].constrainsType" value=""/>
					<input type="hidden" name="listTableDesignDetails[0].displayType"/>
				...
				<td><input id="listTableDesignDetails0.maxlength" name="listTableDesignDetails[0].maxlength" class="form-control qp-input-integer" type="text" value="" maxlength="200"></td>
				...
				<td><a class="btn btn-default btn-xs glyphicon glyphicon-minus qp-button-action" title="Remove table design details" id="deleteRow" onclick="$.qp.removeRowJS('tmp_list_table', this);" style="margin-top: 3px;" href="javascript:void(0)"></a></td>
			</tr>
		</script>
		<script id="tbl_list_Subject-template" type="text/template">
			<tr>
				<td class="qp-output-fixlength tableIndex">1</td>
				<td><qp:autocomplete name="subjectAreas[].areaId" optionValue="optionValue" optionLabel="optionLabel" selectSqlId="getAllSubjectAreaBySubAreaIdForAutocomplete" onChangeEvent="changeTableCodeLabel" arg01="${tableDesignForm.projectId}" mustMatch="true" maxlength="200"></qp:autocomplete></td>
				<td><span></span></td>
				<td><a class="btn btn-default btn-xs glyphicon glyphicon-minus qp-button-action" title='<qp:message code="sc.sys.0014" />' onclick="removeRowEx(this);$.qp.removeRowJSEx(this);" style="margin-top: 3px;" href="javascript:void(0)"></a></td>
			</tr>
		</script>
		<script>
		...
		<form:form method="post" action="${pageContext.request.contextPath}/tabledesign/viewListAffectedChangeDesignForm" modelAttribute="tableDesignForm">
						...
						<tbody class="ui-sortable">
							<c:forEach var="subArea" items="${tableDesignForm.subjectAreas}" varStatus="status">
								<tr>
									...
									<td>
										<qp:autocomplete 
											name="subjectAreas[${status.index}].areaId" optionValue="optionValue" optionLabel="optionLabel" selectSqlId="getAllSubjectAreaBySubAreaIdForAutocomplete" value="${subArea.areaId}" displayValue="${subArea.areaIdAutocomplete}" arg01="${tableDesignForm.projectId}" onChangeEvent="changeTableCodeLabel" mustMatch="true" maxlength="200">
										</qp:autocomplete>
									</td>
							...
							<c:forEach var="item" items="${tableDesignForm.listTableDesignDetails}" varStatus="status">
								<tr>
									...
									<td>
										...
										<form:hidden path="listTableDesignDetails[${status.index}].columnId" value="${item.columnId}"/>
										   		...
										   		<c:forEach var="typeDisplay" items="${tableDesignForm.listGroupOfDatatype}" >
												   		<optgroup label="${typeDisplay.label}">
												   			<c:forEach var="dataType" items="${typeDisplay.listOfDataType}" >
												   				<form:option primitiveId = "${dataType.primitiveId}" value="${dataType.basetypeId}" codelistCodeAutocomplete="${dataType.codelistCodeAutocomplete}" codelistDefaultAutocomplete="${dataType.codelistDefaultAutocomplete}" sqlCodeAutocomplete="${dataType.sqlCodeAutocomplete}" baseTypeGroup="${dataType.groupBaseTypeId}" datatypeFlg="${typeDisplay.datatypeFlg}" maxValue="${dataType.maxValue}" minValue="${dataType.minValue}" validationRule="${dataType.validationRule}" length="${dataType.length}" precision="${dataType.precision}" mandatory="${dataType.mandatory}" remark="${dataType.remark}" defaultValue="${dataType.defaultValue}" supportOptionFlg="${dataType.supportOptionFlg}" userDefineValue="${dataType.userDefineValue}" operatorCode="${dataType.operatorCode}" datasourceType="${dataType.datasourceType}" datasourceId="${dataType.datasourceId}" constrainsType="${dataType.constrainsType}" style = "background-color: ${dataType.color}">${dataType.label}</form:option>
												   			</c:forEach>
												   		</optgroup>
										   		</c:forEach>
									   		</form:select>
									...
									<td align="center"><form:checkbox path="listTableDesignDetails[${status.index}].isMandatory" value="${item.isMandatory}" cssClass="qp-input-checkbox-margin qp-input-checkbox"/>
										...
										<input type="hidden" name="mandatoryOfTable" value="${item.isMandatory}">
									</td>
									<td align="center">
										<span class="glyphicon glyphicon-comment icon-open-dialog-show" title="Remark setting" id="2" data-target="#dialogAutocomplete" onclick="openDialogSettingRemarkColumn(this);" ></span>
			...
			<div class="qp-div-action">
				<qp:authorization permission="tabledesignModify">
					...
					<div class="form-inline form-group">
						...
						<button type="submit" onclick="submitForm" class="btn qp-button qp-dialog-confirm"><qp:message code="sc.sys.0031"/></button>
					</div>
				</qp:authorization>
			</div>
		</c:if>

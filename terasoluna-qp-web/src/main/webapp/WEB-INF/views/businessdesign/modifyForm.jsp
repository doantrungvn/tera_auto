<tiles:insertDefinition name="layouts">
	...
	<tiles:putAttribute name="breadcrumb">
		...
		<li><span><qp:message code="sc.businesslogicdesign.0046" /></span></li>
	</tiles:putAttribute>
						...
						<table class="table table-bordered qp-table-form" id="tbl-blogicinformation">
							...
							<tr>
								<c:if test="${businessDesignForm.requestMethod == 4}">
									<!-- in case of not setting screen, User can set again -->
									<c:if test="${not empty businessDesignForm.screenId}">
									...
									<c:if test="${empty businessDesignForm.screenId}">
										<th>
											<qp:message code="sc.businesslogicdesign.0257" />&nbsp;
										...
										<td>
											<qp:autocomplete optionValue="optionValue" optionLabel="optionLabel" selectSqlId="getAllScreenByFunctionDesignIdForAutocomplete" 
												emptyLabel="sc.sys.0030" name="screenId" arg01="${sessionScope.CURRENT_PROJECT.projectId}" arg02="20" arg03="${businessDesignForm.moduleId}" displayValue="${businessDesignForm.screenIdAutocomplete}" onChangeEvent="onchangeScreenDesign" value="${businessDesignForm.screenId}" />
										</td>
									</c:if>
									<!-- in case of not setting form, User can set again -->
									<c:if test="${not empty businessDesignForm.screenFormId}">
										<form:hidden path="screenFormId" />
										<form:hidden path="screenFormIdAutocomplete" />
										<th><qp:message code="sc.businesslogicdesign.0251" /></th>
										<td><qp:formatText value="${businessDesignForm.screenFormIdAutocomplete}" /></td>
									</c:if>
									<c:if test="${empty businessDesignForm.screenFormId}">
										<th><qp:message code="sc.businesslogicdesign.0251" />&nbsp;<label class="qp-required-field"><qp:message code="sc.sys.0029" /></label></th>
										<td>
								...
								<c:if test="${businessDesignForm.requestMethod != 4}">
									...
									<c:if test="${not empty businessDesignForm.screenId}">
										...
										<td colspan="3">
											<qp:formatText value="${businessDesignForm.screenIdAutocomplete}" />
										</td>
									</c:if>
									<c:if test="${empty businessDesignForm.screenId}">
										<th>
											<qp:message code="sc.businesslogicdesign.0256" />&nbsp;
											<a style="margin-left:5px" class="glyphicon glyphicon-info-sign qp-link-toggle" title='<qp:message code="sc.businesslogicdesign.0258"></qp:message>' data-html="true" data-toggle="tooltip" data-placement="top"></a>
										</th>
										<td>
											<qp:autocomplete optionValue="optionValue" optionLabel="optionLabel" selectSqlId="getAllScreenByFunctionDesignIdForAutocomplete" 
												emptyLabel="sc.sys.0030" name="screenId" arg01="${sessionScope.CURRENT_PROJECT.projectId}" arg02="20" arg03="${businessDesignForm.functionDesignId}" displayValue="${businessDesignForm.screenIdAutocomplete}" onChangeEvent="onchangeScreenDesign" value="${businessDesignForm.screenId}" />
										</td>
									</c:if>
								</c:if>
							</tr>
						...
						<c:if test="${businessDesignForm.clientCheckFlg}">
							...
							<div id="tabBusiness-clientcheck" class="tab-pane" style="height: auto;">
								<div class="panel panel-default qp-div-select">
									<div class="panel-heading">
										<span class="glyphicon  qp-heading-icon" aria-hidden="true">&nbsp;</span> <span class="pq-heading-text"><qp:message code="sc.businesslogicdesign.0049" /></span>
									</div>
									<div class="panel-body">
													...
													<c:forEach items="${businessDesignForm.lstItemValidations }" var="screenItemChecks" varStatus="status">
														<tr>
															...
															<c:if test="${screenItemChecks.areaRowspan > 0 }">
																<td rowspan="${screenItemChecks.areaRowspan}">${screenItemChecks.areaName}/<qp:message code="${CL_SCREEN_AREA_TYPE.get(screenItemChecks.areaType.toString())}" /></td>
															</c:if>
															<td>${screenItemChecks.itemCode}</td>
															<td><c:if test="${screenItemChecks.mandatoryFlg}">

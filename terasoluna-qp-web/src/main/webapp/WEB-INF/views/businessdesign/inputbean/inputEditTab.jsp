		...
		<div class="panel-body">
			<table class="table table-bordered qp-table-list" id="tbl_inputbean_list_define" data-ar-dataClass="ar-dataRow" data-ar-callback="$.qp.businessdesign.inputFormCallback">
				<colgroup>
					...
					<col width="180px" />
                    <col width="30%" class="bd-in-screenitem-hidden"/>
				...
				<tbody>
					<c:forEach items="${businessDesignForm.lstInputBean }" var="inputObj" varStatus="status">
						<c:choose>
							<c:when test="${inputObj.inputBeanType == 0}">
								<tr data-ar-rgroup="${inputObj.groupId}" class="ar-dataRow" data-ar-templateid="tbl_inputbean_list_define-template" data-ar-rindex="${inputObj.itemSequenceNo }" data-ar-rgroupindex="${inputObj.tableIndex }">
										...
										<div style="height: 100%">
											...
											<div class="pull-right" style="height:100%">	
												...
												<div style="width:98%; display: inline-block; vertical-align: middle;">
													<form:hidden path="lstInputBean[${status.index}].createdMessageFlg" />
													<form:hidden path="lstInputBean[${status.index}].inputBeanName" />
													<form:hidden path="lstInputBean[${status.index}].messageStringAutocomplete" />
													...
													<qp:formatText value="${inputObj.messageStringAutocomplete}"></qp:formatText>
												</div>
											</div>
										</div>
									</td>
									<td><form:hidden path="lstInputBean[${status.index}].inputBeanId" /> <form:hidden path="lstInputBean[${status.index}].parentInputBeanId" /> <form:hidden path="lstInputBean[${status.index}].itemSequenceNo" /> <form:hidden path="lstInputBean[${status.index}].inputBeanCode" /> <form:hidden path="lstInputBean[${status.index}].screenItemId" /> <form:hidden path="lstInputBean[${status.index}].inputBeanType" /> <qp:formatText value="${inputObj.inputBeanCode}"></qp:formatText></td>
									<td>
										<div class="input-group">
											<form:hidden path="lstInputBean[${status.index}].dataType" />
											<form:hidden path="lstInputBean[${status.index}].arrayFlg" />
											<qp:formatText value="${CL_BD_DATATYPE.get(inputObj.dataType.toString())}"></qp:formatText>
                                    ...
                                    <td class="bd-in-screenitem-hidden">
                                        <input type="hidden" name="lstInputBean[${status.index}].screenItemMapping.screenItemId" value="${inputObj.screenItemMapping.screenItemId}" mappingType="${inputObj.screenItemMapping.mappingType}"/>
                                        <input type="hidden" name="lstInputBean[${status.index}].screenItemMapping.mappingType" value="${inputObj.screenItemMapping.mappingType}" />
                                        <input type="hidden" name="lstInputBean[${status.index}].screenItemMapping.itemName" value="${inputObj.screenItemMapping.itemName}" />
                                        ...
                                        <c:choose>
											<c:when test="${inputObj.screenItemId != null}">
												<span title='<qp:message code="sc.businesslogicdesign.0261" />' class="btn btn-default btn-xs glyphicon glyphicon-list-alt qp-button-action bd-in-screenitem-hidden qp-cursor" style="float: right; margin-right: 5px;" onclick="openModalAssignScreenItem(this,true,0)"></span>
                                        	</c:when>
                                    	</c:choose>
                                    </td>
										...
										<div class="dropdown">
											...
											<ul class="dropdown-menu dropdown-nav-left">
												<li>
                                                    ...
                                                    <form:hidden path="lstInputBean[${status.index}].dataTypeSession" />
                                                    <form:hidden path="lstInputBean[${status.index}].arrayFlagSession" />
<%--                                                     <c:if test="${inputObj.screenItemId == null}"> --%>
<%-- 														<a class="qp-link qp-cursor" onclick="openDialogGetScope(this,false)"><qp:message code="sc.businesslogicdesign.0262"/></a> --%>
<%-- 													</c:if> --%>
												</li>
												<li>
									...
									<form:hidden path="lstInputBean[${status.index}].mappingScreenItemFlag"/>
									<td>
										<c:if test="${inputObj.objectType != 3}">
											<c:choose>
												<c:when test="${! inputObj.mappingScreenItemFlag}">
													<a class="btn btn-default btn-xs glyphicon glyphicon-minus qp-button-action" style="border-color: red;" onclick="$.qp.ar.removeRow({link:this})" title='<qp:message code="sc.sys.0014" />'></a>
												</c:when>
												<c:otherwise>

<tiles:insertDefinition name="layouts">
	...
	<tiles:putAttribute name="breadcrumb">
		...
		<li><span><qp:message code="sc.businesslogicdesign.0226"/></span></li>
	</tiles:putAttribute>
	...
	<tiles:putAttribute name="body">
		...
		<form:form method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/businessdesign/registerWS" modelAttribute="businessDesignForm">
				...
				<div class="panel-body">
					<table class="table table-bordered qp-table-form" id="tbl-blogicinformation">
						...
						<tr>
							...
							<td>
								<c:forEach var="item" items="${CL_RETURN_TYPE_WEBSERVICE}">
									<label><form:radiobutton path="returnType" value="${item.key}" class="qp-input-radio-margin  qp-input-radio"/>${item.value}</label>
								</c:forEach>
							</td>
							<th><form:label path="httpStatus">HTTP Status&nbsp;<label class="qp-required-field"><qp:message code="sc.sys.0029" /></label></form:label></th>
							<td>
								<form:select path="httpStatus" class="form-control qp-input-select">
									<form:options path="httpStatus" items="${CL_HTTP_STATUS}" />
								</form:select>
							</td>
						</tr>
						<tr>
							...
							<td>
								<c:forEach var="item" items="${CL_REQUEST_METHOD_WEBSERVICE}">
									<label><form:radiobutton path="requestMethod" value="${item.key}" class="qp-input-radio-margin  qp-input-radio"/><qp:message code="${item.value}" /></label>
								</c:forEach>
							</td>
							<th><form:label path="authenticatedFlg"><qp:message code="sc.businesslogicdesign.0228"/></form:label></th>
							<td><form:checkbox path="authenticatedFlg" onchange="setURLOfWS()" cssClass="qp-input-checkbox-margin qp-input-checkbox" /></td>
						</tr>
						<tr>
			...
			<div id="tabs">
				...
				<div class="tab-content">
					...
					<jsp:include page="inputbean/inputWSTab.jsp" />
					<!-- End Input Bean(in.) Tab  -->

					<!-- Begin Output Bean(out.) Tab  -->
					<jsp:include page="outputbean/outputWSTab.jsp" />

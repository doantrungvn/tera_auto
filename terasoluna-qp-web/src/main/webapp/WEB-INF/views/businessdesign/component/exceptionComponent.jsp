<!-- Dialog exception setting -->
<div id="dialog-exception-setting" class="modal fade" style="display: none;">
	<script id="tbl-exception-inputbean-template" type="text/x-jquery-tmpl">
		<tr>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="panel panel-default  qp-div-information">
					<div class="panel-heading">
						...
						<span class="pq-heading-text"><qp:message code="sc.blogiccomponent.0196"/></span>
					</div>
					<div class="panel-body">
						<table class="table table-bordered qp-table-form">
							...
							<tbody>
								...
								<tr>
									<th><qp:message code="sc.blogiccomponent.0199"/></th>
									<td colspan="1" name="exceptionToType">
										<label><input type="radio" name="exceptionToType" value="0" class="qp-input-radio qp-input-radio-margin" onchange="onChangeExceptionType(this)"/>Current business logic</label>
										<label><input type="radio" name="exceptionToType" value="1" class="qp-input-radio qp-input-radio-margin" onchange="onChangeExceptionType(this)"/><qp:message code="sc.businesslogicdesign.0135"/></label>
										<label><input type="radio" name="exceptionToType" value="2" class="qp-input-radio qp-input-radio-margin" onchange="onChangeExceptionType(this)"/><qp:message code="sc.sys.0058"/></label>
									</td>
								...
								<tr id='trModule'>
									...
									<td>
										<qp:autocomplete optionValue="optionValue" optionLabel="optionLabel" selectSqlId="getAllModuleByModuleNameAndProjectIdForAutocomplete" emptyLabel='sc.sys.0030' name="moduleId" 
											arg01="${sessionScope.CURRENT_PROJECT.projectId}" arg02 = "20" arg03 = "1" onChangeEvent="onchangeModuleOfException" >
										</qp:autocomplete>
									</td>
									<th>
										<label><qp:message code="sc.blogiccomponent.0198"/></label>&nbsp;<label class="qp-required-field"><qp:message code="sc.sys.0029" /></label>
										<a name="registerNavigatorLink" style="cursor: pointer;">(<qp:message code="sc.sys.0005"/>)</a></th>
									<td id='tdTypeBlogic'>
										<qp:autocomplete name="exceptionToId" optionValue="optionValue" optionLabel="optionLabel" selectSqlId="getAutocompleteBusinessLogicForexceptionBD" arg01="${f:h(sessionScope.CURRENT_PROJECT.projectId)}" arg02="20" arg03="${businessDesignForm.moduleId}" arg04="${businessDesignForm.businessLogicId}" onChangeEvent="onchangeNavigatorOfException"></qp:autocomplete>
									</td>
                                    <td id='tdTypeCommon'>
                                        <select class="form-control qp-input-select" name="exceptionToId">
                                            <c:forEach var="item" items="${CL_COMMON_ERROR}">
                                                <option value="${item.key}">${item.value}</option>
                                            </c:forEach>
                                        </select>
                                    </td>
								</tr>
                                <tr style="display: none;"><th><label><qp:message code="sc.businesslogicdesign.0101"/> </label></th>
                                    <td>
                                        <select class="form-control qp-input-select" name="type" onchange="">
						...
						<br />
						<table class="table table-bordered qp-table-list" id="tbl-exception-inputbean">
							<colgroup>
								<col />
								<col width="30%" />
								<col width="15%" />
			...
			<div class="modal-footer">	
				<div class="qp-div-action">
        			...
        			<button type="button" class="btn qp-button-client " onclick="saveModalExceptionSetting(this)"><qp:message code="sc.sys.0031"></qp:message></button>
			</div>
			</div>
		</div>
	</div>

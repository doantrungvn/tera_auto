...
function saveDialogComponentListSetting(modalName) {
	...
	if (localCodelist == 3){
		...
		$(modal).find(divtablename).find("tbody").find("tr").each(function (i){
			if($(this).find("[name=parameterOptionName]").html() != undefined && $(this).find("[name=parameterOptionValue]").html() != undefined){
				var parameterOptionName = "";
...
//	var isEnable  = "";
//	if($(modal).find("input[name='enable']") != undefined){
//		isEnable =  $(modal).find("input[name='enable']").prop("checked");
//	}
...
function saveDialogButtonAreaSetting(obj,modalName) {
	...
	if(!validateRequired(modal,"navigateToAutocomplete",dbMsgSource['sc.screendesign.0068'])) {
		return;
	}
	...
	var navigateTo  = "";
	if($(modal).find("input[name='navigateTo']").val() != undefined){
		navigateTo = $(modal).find("input[name='navigateTo']").val();
	}
	
	var navigateToText  = "";
	if($(modal).find("input[name='navigateToAutocomplete']").val() != undefined){
		navigateToText = $(modal).find("input[name='navigateToAutocomplete']").val();
	}	
	...
	var parameters = "";
	$(modal).find("#dialog-button-area-setting-tbl-parameter").find("tr").each(function (i){
		if($(this).find("input[name=parameterAttribute]").val() != undefined && $(this).find("input[name=parameterAttribute]").val().length > 0){
			var tableCode = "";
			var tableCodeAutocomplete = "";
			if($(this).find("input[name=tableCode]").val() != undefined && $(this).find("input[name=tableCode]").val().length > 0){
			...
			parameters += $(this).find("input[name=parameterAttribute]").val()
			+ "π" + tableCode
			+ "π" + tableCodeAutocomplete
			+ "π" + columnCode
			+ "π" + columnCodeAutocomplete
...
function saveDialogLinkAreaSetting(obj, modalName){
	...
	var parameters = "";
	$(modal).find("#tbl-hiddenParameter").find("tr").each(function (i){
		if($(this).find("input[name=parameterAttribute]").val() != undefined && $(this).find("input[name=parameterAttribute]").val().length > 0){
			var tableCode = "";
			var tableCodeAutocomplete = "";
			if($(this).find("input[name=tableCode]").val() != undefined && $(this).find("input[name=tableCode]").val().length > 0){
...
function myFunction() {
    ...
    if (1 == x) {  //Check condition here
        $('#eff_type').empty();
        $('#eff_type').append('<option value="1">Single entity</option>');
        $('#eff_type').append('<option value="2">List enity</option>');
        $('#eff_type').append('<option value="3">Item</option>');
    }
    if(2 == x){
    	$('#eff_type').empty();
        $('#eff_type').append('<option value="1">Single entity</option>');
    ...
    if(3 == x){
    	$("#idAction-choose").parent().next().toggle();
    }
}

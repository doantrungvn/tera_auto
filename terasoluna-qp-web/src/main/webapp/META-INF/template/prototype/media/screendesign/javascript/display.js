		...
		for (var i = 0; i < outputBeans.length; i++) {
			...
			var content = '<tr beanid="'+outputBeans[i].outputBeanId+'" level="'+outputBeans[i].tableIndex+'" isobjectarray="'+outputBeans[i].arrayFlg+'"  isobjectdatatype="'+outputBeans[i].dataType+'" data-ar-rgroup="'+outputBeans[i].groupId+'">' +
								...
								'<div style="height: 100%;">' +
									...
									'<div style="float:left;height:100%;vertical-align: middle; padding: 6px 7px;">' +
										outputBeans[i].messageStringAutocomplete+
									'</div>' +
								'</div>' +
							'</td>' +
							'<td>' +
...
function openSettingDefaultValue(obj, datatype) {
	...
	if (datatype == '9') {
		...
		$("#settingDefaultValue").find("input[name=defaultValue][value=5]").closest("label").css({"display":"inline-block"});
		$("#settingDefaultValue").find("input[name=defaultValue][value=1]").closest("label").hide();
		$("#settingDefaultValue").find("input[name=defaultValue][value=2]").closest("label").hide();
		$("#settingDefaultValue").find("input[name=defaultValue][value=3]").closest("label").hide();
	} else {

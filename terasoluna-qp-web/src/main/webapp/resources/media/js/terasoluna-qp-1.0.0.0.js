var escapseHTMLentityMap = {
 ...
 var REGULAR_EXP_CODE = /^[a-zA-Z][_0-9a-zA-Z]{0,}$/;
(function() {
	...
	$.qp.getMessage = function(messageCode) {
		if(typeof fcomMsgSource != 'undefined')
			return $.qp.unEscapseHTML(fcomMsgSource[messageCode]);
		else
			return messageCode;
	}
	
	$.qp.isNullOrEmpty = function(value) {
			...
			$(autocomplete).autocomplete({				
				source: function( request, response ) {
			      ...
			      open: function(event, ui) {
			    	  ...
			    	  var position = $(".ui-autocomplete:visible").offset();
			    	  var elementHeight = $(".ui-autocomplete:visible").outerHeight();
			    	  var height = $( window ).height();
			    	  
			    	  if ((position.top - scrollTop) + elementHeight > height - 20) {
			    		  $(".ui-autocomplete:visible").height(elementHeight - ((position.top - scrollTop) + elementHeight - height) - 15);
			    	  } else {
			    		  $(".ui-autocomplete:visible").height('');
			    	  }
			    	  // show more results
				...
				source: function( request, response ) {
			      ...
			      change: function (event, ui) {
			    	  ...
			    	  for (var i = 1; i < 20; i++){
			    		  if (i < 10) {
			    			  var name = "output0" + i;
			    			  if (ui.item != undefined && ui.item != null && ui.item[name] != undefined && ui.item[name] != null && ui.item[name].length > 0) {
			    				  $(autocomplete).next("input[type=hidden]").attr(name, ui.item[name]);
			    			  } else {
			    				  $(autocomplete).next("input[type=hidden]").removeAttr(name);
			    			  }
	...
	$.qp.removeAutocompleteData = function(obj){
		...
		//TungHT 
		if($(obj).prev("input[type=hidden]").prev("input[name='screenTransitionAutocomplete") != undefined 
				&& $(obj).prev("input[type=hidden]").prev("input[name='screenTransitionAutocomplete") != "" 
				&& $(obj).prev("input[type=hidden]").prev("input[name='screenTransitionAutocomplete").length > 0) {
			$(obj).prev("input[type=hidden]").prev("input[name='screenTransitionAutocomplete").attr("arg04","");
		}
	...
	$.qp.initialAutoNumeric = function(thiz) {
		...
		$.qp.formatInteger($(thiz).find('.qp-input-from-integer'));
		$.qp.formatInteger($(thiz).find('.qp-input-to-integer'));
		
		$.qp.formatBigint($(thiz).find('.qp-input-bigint'));
		$.qp.formatBigint($(thiz).find('.qp-input-from-bigint'));
		$.qp.formatBigint($(thiz).find('.qp-input-to-bigint'));
		
		$.qp.formatSerial($(thiz).find('.qp-input-serial'));
		$.qp.formatSerial($(thiz).find('.qp-input-from-serial'));
		$.qp.formatSerial($(thiz).find('.qp-input-to-serial'));
		
		$.qp.formatBSerial($(thiz).find('.qp-input-bigserial'));
		$.qp.formatBSerial($(thiz).find('.qp-input-from-bigserial'));
		$.qp.formatBSerial($(thiz).find('.qp-input-to-bigserial'));
		
		$.qp.formatSmallint($(thiz).find('.qp-input-smallint'));
		$.qp.formatSmallint($(thiz).find('.qp-input-from-smallint'));
		$.qp.formatSmallint($(thiz).find('.qp-input-to-smallint'));
		
		$.qp.formatFloat($(thiz).find('.qp-input-float'));
		$.qp.formatFloat($(thiz).find('.qp-input-from-float'));
		$.qp.formatFloat($(thiz).find('.qp-input-to-float'));
		
		$.qp.formatCurrency($(thiz).find('.qp-input-currency'));
		$.qp.formatCurrency($(thiz).find('.qp-input-from-currency'));
		$.qp.formatCurrency($(thiz).find('.qp-input-to-currency'));
		
		$.qp.formatPercentage($(thiz).find('.qp-input-percentage'));
		$.qp.formatPercentage($(thiz).find('.qp-input-from-percentage'));
		$.qp.formatPercentage($(thiz).find('.qp-input-to-percentage'));
		
		$.qp.formatPercentageDecimal($(thiz).find('.qp-input-percentage-decimal'));
		$.qp.formatPercentageDecimal($(thiz).find('.qp-input-from-percentage-decimal'));
		$.qp.formatPercentageDecimal($(thiz).find('.qp-input-to-percentage-decimal'));
	};

	$.qp.initialAllPicker = function(thiz) {
		$.qp.initialDateTimePicker($(thiz).find(".qp-input-datetimepicker"));
		$.qp.initialDateTimePicker($(thiz).find(".qp-input-from-datetimepicker"));
		$.qp.initialDateTimePicker($(thiz).find(".qp-input-to-datetimepicker"));	
		
		$.qp.initialDateTimePickerDetail($(thiz).find(".qp-input-datetimepicker-detail"));
		$.qp.initialDateTimePickerDetail($(thiz).find(".qp-input-from-datetimepicker-detail"));
		$.qp.initialDateTimePickerDetail($(thiz).find(".qp-input-to-datetimepicker-detail"));
		
		$.qp.initialDatePicker($(thiz).find(".qp-input-datepicker"));
		$.qp.initialDatePicker($(thiz).find(".qp-input-from-datepicker"));
		$.qp.initialDatePicker($(thiz).find(".qp-input-to-datepicker"));
		
		$.qp.initialTimePicker($(thiz).find(".qp-input-timepicker"));
		$.qp.initialTimePicker($(thiz).find(".qp-input-from-timepicker"));
		$.qp.initialTimePicker($(thiz).find(".qp-input-to-timepicker"));
		
		$.qp.initialTimePicker($(thiz).find(".qp-input-timepicker-detail"));
		$.qp.initialTimePicker($(thiz).find(".qp-input-from-timepicker-detail"));
		$.qp.initialTimePicker($(thiz).find(".qp-input-to-timepicker-detail"));

		$.qp.initialAutocomplete($(thiz).find(".qp-input-autocomplete"));
	...
	$.qp.formatBigint = function(thiz) {
		...
		if ($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "-9223372036854775808", "9223372036854775807");
	...
	$.qp.formatSmallint = function(thiz) {
		if ($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "-32768", "32767");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatSerial = function(thiz) {
		if ($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "0", "2147483647");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatBSerial = function(thiz) {
		if ($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "0", "9223372036854775807");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatInteger = function(thiz) {
		if ($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "-2147483648", "2147483647");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatFloat = function(thiz) {
		if ($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "-999999999999.99999", "999999999999.99999");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatCurrency = function(thiz) {
		if($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "0", "9999999999999999.999");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatPercentage = function(thiz) {
		if($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "0", "1000");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.formatPercentageDecimal = function(thiz) {
		if($(thiz) != undefined && $(thiz).val() != undefined) {
			var minMax = $.qp.getRangeNumber(thiz, "0", "1000.00");
			$(thiz).autoNumeric('init', {
				aSep : ',',
				dGroup : '3',
				aPad : 'false',
	...
	$.qp.showConfirm = function(obj, screenId) {
		var messageConfirm =  $.qp.unEscapseHTML(fcomMsgSource["inf.sys.0015"]);
		...
		if($(obj).attr("message-string") != undefined){
			...
			if($(obj).attr("messageId") != undefined){
				messageConfirm = $.qp.unEscapseHTML(fcomMsgSource[$(obj).attr("messageId")]);
			}
		}
		...
		$(obj).click(function (e) {
			...
			if(passPrecheck){
				...
				if (showConfirmDialogFlg != undefined && showConfirmDialogFlg == "false") {
					...
					var messageCustom = $(obj).attr("messageCustom") || "";
					if(messageCustom != ""){
						$("#fcomConfirmDialog .modal-body").html(messageCustom);
					}
	...
	$.qp.addRowJSByLink = function(link) {
		//var table = $(link).closest("div").prev("table");
		//var tableId = $(link).closest("div").prev("table").attr("id");
		var template = $(link).closest("div").prev("table").attr("id") + "-template";
		...
		$.qp.addRowJSByLinkEx(link, false, template);
			...
			if (table.find("tr").size() == 0) {
				...
				if (callback != undefined) {
					if(typeof window[callback]=="function"){
						window[callback]($table,'add',tmpl);
					}
				}
			}
			$.qp.initialAutoNumeric(obj);// initial Auto numeric
...
//			$navbar.find("a.navbar-brand").attr("href","javascript:void(0)");
	...
	$.qp.initialConventionNameCode = function(obj) {
		if(obj == undefined || obj == null){
			$('.qp-convention-name').focusout(function() {
		    	var code = $(this).closest("table").find(".qp-convention-code");
				if ($(code).val() == '')
					$(code).val(($(this).val().capitalize()));
			});
			
			$('.qp-convention-code').focusout(function() {
		    ...
		    $('.qp-convention-name-row').focusout(function() {
		    	var code = $(this).closest("tr").find(".qp-convention-code-row");
				if ($(code).val() == '')
					$(code).val(($(this).val().capitalize()));
			});

			$('.qp-convention-code-row').focusout(function() {
				var name = $(this).closest("tr").find(".qp-convention-name-row");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDash()));
			});
			
			$('.qp-convention-db-name').focusout(function() {
				var code = $(this).closest("table").find(".qp-convention-db-code");
				if ($(code).val() == '')
					$(code).val(($(this).val().toDatabaseCode()));
			});

			$('.qp-convention-db-name-row').focusout('change', function() {
		    	var code = $(this).closest("tr").find(".qp-convention-db-code-row");
				if ($(code).val() == '')
					$(code).val(($(this).val().toDatabaseCode()));
			});
			
			$('.qp-convention-db-code').focusout(function() {
				var name = $(this).closest("table").find(".qp-convention-db-name");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDashDB()).firstTitle());
			});
			
			$('.qp-convention-db-code-row').focusout(function() {
				var name = $(this).closest("tr").find(".qp-convention-db-name-row");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDashDB()).firstTitle());
			});
		}else{
			$(obj).find('.qp-convention-name').focusout(function() {
		    	var code = $(this).closest("table").find(".qp-convention-code");
				if ($(code).val() == '')
					$(code).val(($(this).val().capitalize()));
			});
			
			$(obj).find('.qp-convention-code').focusout(function() {
				var name = $(this).closest("table").find(".qp-convention-name");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDash()));
			});
			...
			//convert name, code in row
			$(obj).find('.qp-convention-name-row').focusout(function() {
		    	var code = $(this).closest("tr").find(".qp-convention-code-row");
				if ($(code).val() == '')
					$(code).val(($(this).val().capitalize()));
			});

			$(obj).find('.qp-convention-code-row').focusout(function() {
				var name = $(this).closest("tr").find(".qp-convention-name-row");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDash()));
			});
			
			$(obj).find('.qp-convention-db-name').focusout(function() {
				var code = $(this).closest("table").find(".qp-convention-db-code");
				if ($(code).val() == '')
					$(code).val(($(this).val().toDatabaseCode()));
			});

			$(obj).find('.qp-convention-db-name-row').focusout(function() {
		    	var code = $(this).closest("tr").find(".qp-convention-db-code-row");
				if ($(code).val() == '')
					$(code).val(($(this).val().toDatabaseCode()));
			});
			
			$(obj).find('.qp-convention-db-code').focusout(function() {
				var name = $(this).closest("table").find(".qp-convention-db-name");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDashDB()).firstTitle());
			});
			
			$(obj).find('.qp-convention-db-code-row').focusout(function() {
				var name = $(this).closest("tr").find(".qp-convention-db-name-row");
				if ($(name).val() == '')
					$(name).val(($(this).val().toDashDB()).firstTitle());
			});
	...
	String.prototype.equalsIgnoreCase = function(strCompare) {
		if (this == undefined || strCompare == undefined) {
			return false
		}
		return this.toLowerCase() === strCompare.toLowerCase();
	}
	
	/** DungNN - 20150813 
	 * add auto generate code to name or name to code only support english
	 * */
	$.qp.validateIsCode = function(value) {
		return REGULAR_EXP_CODE.test(value)
	};
	
	$.qp.showAlertModal = function(message){
		message = $.qp.extractedMessageAlert(message);
		$("#qpModalAlertDialog .modal-body").html(message);
		$.qp.initConfirmModels("#qpModalAlertDialog");
		$("#qpModalAlertDialog").modal({
			 show: true,
			 closable: false,
			 keyboard:false,
			 backdrop:'static'
	...
	$.qp.extractedMessageAlert = function(message){
		var messageTmp = message.replace(/\r\n|\n|\r/g, '<br/>');
		var lstMessage = messageTmp.split("<br/>");
		return lstMessage.filter(function(m){
			return m != "";
		}).join("<br/>");
	};
	
	$.qp.showLoading = function(){
		$.fancybox.showLoading();
		$.fancybox.helpers.overlay.open({parent: $('body'), closeClick : false});
	}
	
	$.qp.hideLoading = function(){

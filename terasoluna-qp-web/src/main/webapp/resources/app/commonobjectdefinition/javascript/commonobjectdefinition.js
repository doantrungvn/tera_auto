...
var mapKeyTemp = [];
function initMapKey() {
	mapKeyTemp["commonobject"] = 0;
	var table = "#tbl_attribute_list_define";
	$(table).find("input[name*='commonObjectAttributeId']").each(function() {
		var id = $(this).val();
		if (id.search("ou") >= 0) {
			id = id.replace("ou","");
			id = parseInt(id);
			if (mapKeyTemp["commonobject"] < id) {
				mapKeyTemp["commonobject"] = id;
			}
		}
	});
}
...
function onChangeExternalObject(event){
	...
	if(event.item != undefined){
		$(obj).closest("tr").find("input[name*='commonObjectAttributeName']").val(event.item.optionLabel);
		$(obj).closest("tr").find("input[name*='commonObjectAttributeCode']").val(event.item.output01);
		$(obj).closest("tr").find("label[name*='commonObjectAttributeCode']").text(event.item.output01);
...
//				});
		...
		$(obj).closest("tr").find("input[name*='commonObjectAttributeName']").val("");
		$(obj).closest("tr").find("input[name*='commonObjectAttributeCode']").val("");
		$(obj).closest("tr").find("label[name*='commonObjectAttributeCode']").text("");
	}
}
function buildExternalObjectAttributes(data, obj) {
    ...
    for(var i=0;i<data.length;i++){
		if (data[i].dataType == 17) {
	    	var actionTemplateId = table+"-common-object-column-template";
			...
			$(newRow).find("input[name*='commonObjectAttributeName']").val(curObj.externalObjectDefinitionName);
			$(newRow).find("label[name*='commonObjectAttributeName']").text(curObj.externalObjectDefinitionName);
			$(newRow).find("input[name*='commonObjectAttributeCode']").val(data[i].externalObjectAttributeCode);
			$(newRow).find("label[name*='commonObjectAttributeCode']").text(data[i].externalObjectAttributeCode);
			$(newRow).find("input[name*='objectDefinitionId']").val(curObj.externalObjectDefinitionId);
			$(newRow).find("input[name*='dataType']").val(data[i].dataType);
			$(newRow).find("input[name*='arrayFlg']").val(data[i].arrayFlg);
			$(newRow).find("label[name*='dataType']").text(CL_EX_DATATYPE[data[i].dataType]+ (data[i].arrayFlg ? "[]" : ""));
			tempAnchor = $(newRow);
			...
			$(newRow).find("input[name*='groupId']").val(rgroup);
			$(newRow).find("input[name*='tableIndex']").text(tableIndex);
			$(newRow).find("input[name*='commonObjectAttributeName']").val(data[i].externalObjectAttributeName);
			$(newRow).find("label[name*='commonObjectAttributeName']").text(data[i].externalObjectAttributeName);
			$(newRow).find("input[name*='commonObjectAttributeCode']").val(data[i].externalObjectAttributeCode);
			$(newRow).find("label[name*='commonObjectAttributeCode']").text(data[i].externalObjectAttributeCode);
			$(newRow).find("input[name*='objectDefinitionId']").val(data[i].externalObjectAttributeId);
			$(newRow).find("input[name*='dataType']").val(data[i].dataType);
			$(newRow).find("input[name*='arrayFlg']").val(data[i].arrayFlg);
			$(newRow).find("label[name*='dataType']").text(CL_EX_DATATYPE[data[i].dataType]+ (data[i].arrayFlg ? "[]" : ""));
			tempAnchor = $(newRow);
...
function onChangeCommonObject(event){
	var obj = event.target;
	var rgroup = $(obj).closest("tr").attr("data-ar-rgroupindex");
	$(obj).closest("tr").nextAll("tr[data-ar-rgroup^='"+rgroup+"']").each(function() {
		$(this).remove();
...
//				});
				...
				tempAnchor = obj;
				buildCommonObjectAttributes(data, obj);
			}
		}
		$(obj).closest("td").find("input[name$='moduleId']").val(event.item.output02);
	}else{
		$(obj).closest("tr").find("input[name*='commonObjectAttributeName']").val("");
		$(obj).closest("tr").find("input[name*='commonObjectAttributeCode']").val("");
		$(obj).closest("tr").find("input[name*='objectDefinitionId']").val("");
	}
}
function buildCommonObjectAttributes(data, obj) {
	var tempParentId = null;
	var rgroup = $(obj).closest("tr").attr("data-ar-rgroupindex");
	var table = $(obj).closest("table").attr("id");
	var mapParentId = [];
...
//		}
		...
		if (data[i].dataType == 16) {
			...
			$(newRow).find("input[name*='groupId']").val(rgroup);
			$(newRow).find("input[name*='tableIndex']").text(tableIndex);
			$(newRow).find("input[name*='commonObjectAttributeName']").val(data[i].commonObjectAttributeName);
			$(newRow).find("label[name*='commonObjectAttributeName']").text(data[i].commonObjectAttributeName);
			$(newRow).find("input[name*='commonObjectAttributeCode']").val(data[i].commonObjectAttributeCode);
			$(newRow).find("label[name*='commonObjectAttributeCode']").text(data[i].commonObjectAttributeCode);
			$(newRow).find("input[name*='objectDefinitionId']").val(data[i].objectDefinitionId);
			$(newRow).find("input[name*='dataType']").val(data[i].dataType);
			$(newRow).find("input[name*='arrayFlg']").val(data[i].arrayFlg);
			$(newRow).find("label[name*='dataType']").text(CL_EX_DATATYPE[data[i].dataType] + (data[i].arrayFlg ? "[]" : ""));
			tempAnchor = $(newRow);
			
			buildCommonObjectAttributes(data[i].commonObjectDefinition.commonObjectAttributes, newRow);
			...
			$(newRow).find("input[name*='commonObjectAttributeCode']").val(data[i].commonObjectAttributeCode);
			$(newRow).find("label[name*='commonObjectAttributeCode']").text(data[i].commonObjectAttributeCode);
			$(newRow).find("input[name*='objectDefinitionId']").val(data[i].commonObjectAttributeId);
			$(newRow).find("input[name*='dataType']").val(data[i].dataType);
			$(newRow).find("input[name*='arrayFlg']").val(data[i].arrayFlg);
			$(newRow).find("label[name*='dataType']").text(CL_EX_DATATYPE[data[i].dataType] + (data[i].arrayFlg ? "[]" : ""));
			tempAnchor = $(newRow);
...
function objectTypeChange(obj){
	...
	var commonObjectAttributeCode = $ParentRow.find("input[name$='commonObjectAttributeCode']").val();
	...
	if(newRow != undefined){
//		changeArgOfRowObjByType(newRow, type, $('table[id="obj-infor-master-tbl"]').find("input:hidden[name='moduleId']").val());
		$(newRow).find("input[name$='commonObjectAttributeCode']").val(commonObjectAttributeCode);
	}
}

function saveCommonOject() {
	var table = "#tbl_attribute_list_define";
	...
	if (msg != "") {
		...
		$(table).find(">tbody>tr.ar-dataRow").each(function(i) {
			...
			var id = $(this).find("input[name*='commonObjectAttributeId']").val();
			if (id == "") {
				id = mapKeyTemp["commonobject"];
				mapKeyTemp["commonobject"] = parseInt(id) + 1;
				id = "co" + mapKeyTemp["commonobject"];
			}
			mapKey[rgroup] = id;
			$(this).find("input[name*='commonObjectAttributeId']").val(id);
			...
			if (arrayIndex.length > 1) {
				...
				$(this).find("input[name*='parentCommonObjectAttributeId']").val(mapKey[keyParent]);
			} else {
			}
...
function addRow(param){
	...
	if($container!="undefined") {
		...
		$newRow.find("select[name$='.dataType'] option[value='16']").remove();
		...
		if(passCheck && $newRow.toString()){
			...
			$.qp.ar.recalculateRowIndex($container,param.indexClass,templateData.groupId);
...
function validateBeforeSubmit(table) {
	...
	var messages="";
	messages += validateRequired(table,"commonObjectAttributeName", ITEM_NAME.ATTRIBUTE_NAME);
	messages += validateRequired(table,"commonObjectAttributeCode", ITEM_NAME.ATTRIBUTE_CODE);
	messages += validateRequired(table,"dataType", ITEM_NAME.DATATYPE);
	messages += validateSpecialChar(table,"commonObjectAttributeCode", ITEM_NAME.ATTRIBUTE_CODE);
//	messages += validateDuplicate(table,"commonObjectAttributeName", ITEM_NAME.ATTRIBUTE_NAME);
	messages += validateDuplicate(table,"commonObjectAttributeCode", ITEM_NAME.ATTRIBUTE_CODE);
	return messages;
}
...
function changeArgOfRowObjByType(newRow, type, argValue) {
	if($(newRow) != undefined && $(newRow).find("input:text[name$='.objectDefinitionIdAutocomplete']").val() != undefined && (type == "16" || type == "17")){
		$(newRow).find("input:text[name$='.objectDefinitionIdAutocomplete']").val('');
		$(newRow).find("input:hidden[name$='.objectDefinitionId']").val('');
		$(newRow).find("input:text[name$='.objectDefinitionIdAutocomplete']").attr("arg04", argValue=(argValue != undefined?argValue:""));

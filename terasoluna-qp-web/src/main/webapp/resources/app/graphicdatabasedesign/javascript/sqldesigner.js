...
SQL.Row.prototype.onChangeType = function() {
	...
	{
		case DATATYPE.TEXT:
			if (parseInt(selectedType) == CHARACTER_TYPE.TEXT) {
...
SQL.Row.prototype.expand = function() {
	...
	var table = this.owner.data;
	if (TABLE_TYPE.COMMON == table.tableType ) {
		$.qp.showAlertModal(dbMsgSource["err.databasedesign.0003"].replace("{0}",dbMsgSource["sc.databasedesign.0046"])
				.replace("{1}",dbMsgSource["sc.databasedesign.0048"])
		);
	} else	if (DESIGN_STATUS.FIXED == table.tableStatus) {
		$.qp.showAlertModal(dbMsgSource["err.databasedesign.0003"].replace("{0}",dbMsgSource["sc.databasedesign.0046"])
				.replace("{1}",dbMsgSource["sc.databasedesign.0049"])
		);
	}
	...
	var thiz = this;
	$(this.dom.def).on("focus", function(e) {
		if ($(this).hasClass("qp-input-float")) {
			var maxlength = $(thiz.dom.size).val();
			var precision = $(thiz.dom.precision).val();
			/*console.log(initializeMaxlength(maxlength, precision));
			console.log(initializeMinlength(maxlength, precision));*/
			
			$(this).attr("qp-max", initializeMaxlength(maxlength, precision));
			$(this).attr("qp-min", initializeMinlength(maxlength, precision));
			$(this).autoNumeric('destroy');
			$.qp.formatFloat(this);
		} else if ($(this).hasClass("qp-input-currency")) {
			var maxlength = $(thiz.dom.size).val();
			var precision = $(thiz.dom.precision).val();
...
SQL.Row.prototype.collapse = function(e) {
	...
	var table = this.owner.data;
	if (TABLE_TYPE.COMMON == table.tableType || DESIGN_STATUS.FIXED == table.tableStatus) {
		OZ.DOM.clear(this.dom.container);
		this.dom.container.appendChild(this.dom.content);
		return;
	}
	
	/*if (!this.data.isCommonFlg) {*/
		...
		if ($.qp.isNullOrEmpty(this.dom.code.value)) {
			...
			msg += dbMsgSource["err.databasedesign.0010"].replace("{0}", dbMsgSource["sc.databasedesign.0116"]).replace("{1}", this.owner.getTitle());
			...
			/*if (!$.qp.confirm(fcomMsgSource['err.sys.0130'].replace("{0}",dbMsgSource['sc.databasedesign.0116']))) {
				warning = 1;
			};*/
		...
		var checkExist= validateExistColumn (this.dom.name.value, this.dom.code.value, this.owner.rows);
		if ($.qp.isNullOrEmpty(checkExist) == false) {
			msg += checkExist;
			isValid = 1;
		}

		if (isValid > 0 || warning > 0) {
			if (isValid > 0) {
				/*$.qp.alert(msg);*/
...
SQL.Row.prototype.load = function() { /* put data to expanded form */
	...
	switch (parseInt(groupId))
	{
		...
		case DATATYPE.DECIMAL:
			console.log("DECIMAL");
			this.dom.def.setAttribute("class", "form-control qp-input-float");
			this.dom.def.setAttribute("qp-max", initializeMaxlength(maxlength, precision));
			this.dom.def.setAttribute("qp-min", initializeMinlength(maxlength, precision));
			$.qp.formatFloat(this.dom.def);
			switch (parseInt(baseTypeId))
			{
				case NUMERIC_TYPE.NUMBERIC:
...
SQL.Row.prototype.toXML = function() {
	...
	var isCommonFlg = (this.data.isCommonFlg ? "1" : "0");
	...
	xml += "<isCommonFlg>"+isCommonFlg+"</isCommonFlg>\n";
...
SQL.Row.prototype.fromXML = function(node) {
	...
	var isCommonFlg = node.getElementsByTagName("isCommonFlg");
	if (isCommonFlg.length && isCommonFlg[0].firstChild) { obj.isCommonFlg = (isCommonFlg[0].firstChild.nodeValue == "1"); }
...
SQL.Table.prototype.setTableType = function(tableType) {
...
SQL.Table.prototype.isQpTable = function() {
	return this.data.tableType == TABLE_TYPE.COMMON;
}
...
SQL.Key.prototype.toXML = function() {
	...
	if ($.qp.isNullOrEmpty(this.getName())) {
		...
		var msg = dbMsgSource["err.databasedesign.0010"].replace("{0}", dbMsgSource["sc.databasedesign.0117"]).replace("{1}", this.owner.getTitle());
		msg = msg.replace("{2}", REQUIRED_MIN_INPUT_VAL).replace("{3}", SQL_CODE_MAX_VAL);
		$.qp.showAlertModal(msg);
		return false;
	}
...
SQL.TableManager.prototype.addRow = function(e) {
	...
	if (DESIGN_STATUS.FIXED == this.selection[0].getTableStatus()) {
		$.qp.showAlertModal(dbMsgSource["err.databasedesign.0003"].replace("{0}",dbMsgSource["sc.databasedesign.0046"])
				.replace("{1}",dbMsgSource["sc.databasedesign.0049"])
		);
		return;
...
SQL.TableManager.prototype.edit = function(e) {
	...
	if (this.selection[0].isQpTable()) {
		$("input[name=tableType]").attr('disabled',true);
		this.dom.name.disabled=true;
		this.dom.code.disabled=true;
		this.dom.usedCommonColumn.disabled=true;
		this.dom.comment.disabled=true;
		$("#windowok").hide();
	} else {
		$("input[name=tableType]").attr('disabled',false);
		$("input[name=tableType][value=2]").attr('disabled',true);
		this.dom.name.disabled=false;
		this.dom.code.disabled=false;
		this.dom.usedCommonColumn.disabled=false;
		this.dom.comment.disabled=false;
		$("#windowok").show();
	}
...
SQL.TableManager.prototype.save = function() {
	...
	this.selection[0].setTableType(tableTypeNew);
...
SQL.RowManager.prototype.tableClick = function(e) { /* create relation after clicking target table */
	...
	if (DESIGN_STATUS.FIXED == t2.getTableStatus()) {
		$.qp.showAlertModal($.qp.getMessage("err.sys.0109").replace("{0}", t2.getTitle()));
		return;
	}
...
SQL.RowManager.prototype.rowClick = function(e) { /* draw relation after clicking target row */
	...
	var r2 = e.target;
	/**
	 * DungNN - 20160715 - fix bug 1089 - create fk when table is fixed design
	 */
	var toTable = e.data.owner;
	if (DESIGN_STATUS.FIXED == toTable.getTableStatus()) {
		$.qp.showAlertModal($.qp.getMessage("err.sys.0109").replace("{0}", toTable.getTitle()));
		return;
	}
...
SQL.RowManager.prototype.remove = function(e) {
	...
	if (DESIGN_STATUS.FIXED == t.getTableStatus()) {
		$.qp.showAlertModal(dbMsgSource["err.databasedesign.0003"].replace("{0}",dbMsgSource["sc.databasedesign.0046"])
				.replace("{1}",dbMsgSource["sc.databasedesign.0049"])
		);
		return;
...
SQL.KeyManager.prototype.open = function(table) {
	...
	if (TABLE_TYPE.COMMON == table.data.tableType || DESIGN_STATUS.FIXED == table.data.tableStatus) {
		
		if (TABLE_TYPE.COMMON == table.data.tableType) {
			$.qp.showAlertModal(dbMsgSource["err.databasedesign.0003"].replace("{0}",dbMsgSource["sc.databasedesign.0046"])
					.replace("{1}",dbMsgSource["sc.databasedesign.0048"])
			);
		} else	if (DESIGN_STATUS.FIXED == table.data.tableStatus ) {
			$.qp.showAlertModal(dbMsgSource["err.databasedesign.0003"].replace("{0}",dbMsgSource["sc.databasedesign.0046"])
					.replace("{1}",dbMsgSource["sc.databasedesign.0049"])
			);
		}
...
SQL.Window.prototype.open = function(title, content, callback) {
	...
	if ($(this.dom.container).find(".input-hidden").length > 0) {
		showData();
	}
...
SQL.Designer.prototype.clearTables = function(init) {
	var value = true;
	for (var i=0;i<this.tables.length;i++) {
		var t = this.tables[i];
		if (init || (t.isQpTable() == false && t.getTableStatus() != DESIGN_STATUS.FIXED)) {
			this.removeTable(t);
			value = false;
		}
	}
	if (!value) this.setTitle(value);
}

...
!function($) {
			...
			function handleFlexbox() {
				if (isUsingFlexbox(self)) {
					if (self.element.find('li.tabdrop').hasClass('pull-right')) {
						self.element.find('li.tabdrop').css({
							position : 'absolute',
							right : 0
						});
						self.element.css('padding-right', self.element.find(
								'.tabdrop').outerWidth(true));
					}
				}
			}
			...
			function checkOffsetAndPush(recursion) {
				...
				if (collection.length > 0) {
					...
					if (self.dropdown.find('.active').length == 1) {
						...
						setDropdownText(self.dropdown.find('.active > a')
								.html());
					} else {
	...
	$.fn.tabdrop = function(option) {
		return this
				.each(function() {
					var $this = $(this), data = $this.data('tabdrop'), options = typeof option === 'object'
							&& option;
					if (!data) {

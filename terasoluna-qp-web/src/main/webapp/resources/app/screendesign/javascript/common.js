...
function returnLinkItem(obj){
	...
	if(obj.messageLevel != "" && obj.messageLevel != undefined) {
		messageLevel = obj.messageLevel;
	}
	if(obj.messageLevelText != "" && obj.messageLevelText != undefined) {
		messageLevelText = obj.messageLevelText;
	}
	var element =  "" 
		+ "<input type=\"hidden\" name=\"formElement\" "
		...
		+ "\",\"navigateToBlogic\":\""+obj.navigateToBlogic
		+ "\",\"navigateToBlogicText\":\""+obj.navigateToBlogicText
		...
		+ "\",\"messageConfirm\":\""+obj.messageConfirm
		+ "\",\"messageConfirmText\":\""+obj.messageConfirmText
		+ "\",\"enableConfirm\":\""+obj.enableConfirm
		+ "\",\"messageConfirmCode\":\""+obj.messageConfirmCode
...
function swap(elementSource, elementDest) {
	...
	var table = $(elementDest).closest("table");
	//init saw
...
function returnDivSetting(code){
	return "<div class=\"areaContent\"><div class=\"panel panel-default\"><div class=\"panel-heading\"><span class=\"glyphicon\" aria-hidden=\"true\"></span><span class=\"qp-heading-text\">" +
		...
		'<input type="hidden" value="" name="formDisplayPageable"/>' +
...
function returnTableListSetting(areaType, code, name){
	var areaPatternType = 1;
	if (areaType != null && areaType != undefined) {
		areaPatternType = areaType;
	}
	
	var tableName = dbMsgSource['sc.screendesign.0059'];
...
function returnElementTD(div){	
	...
	var value = "";
	if($(div).attr("data") != undefined){
		value = convertToJson($(div).attr("data"));
		isSupportOptionValue = value.isSupportOptionValue;
	}
	var columnname = "";
	...
	if($(div).attr('msglabel') == undefined || $(div).attr('msglabel') == 'undefined' || $(div).attr('msglabel') == '') {
		$(div).attr('msglabel',msglabel );
	}
	
	var formatcode = "";
	if($(div).attr('datatype') != "" && $(div).attr('datatype') != undefined && $(div).attr('datatype') == "15") {
		formatcode = "Email";
	}
	if($(div).attr('datatype') != "" && $(div).attr('datatype') != undefined && $(div).attr('datatype') == "16") {
		formatcode = "Phone";
	}
	if($(div).attr('datatype') != "" && $(div).attr('datatype') != undefined && $(div).attr('datatype') == "18") {
		formatcode = "Alphanumeric";
	}
	...
	if(dataType == 10){
		elementTD += "<span style=\"width:100%\"><textarea ondblclick=\"openDialogComponentSetting(this)\" ";
					if($(div).attr('elementtype') == 0){
...
function returnElementTHPreview(item, isList){
	...
	if (dataType == 20) {
		var element =  "<span><label style=\"cursor: pointer;"+floatLeft+ ' ' + item.headerStyle+ item.style + "\">" + item.labelText + "</label>";
	} else {
		var element =  "<span><label style=\"cursor: pointer;"+floatLeft+ ' ' + item.headerStyle+ item.style + " width: "+width+""+widthunit+"\">" + item.labelText + "</label>";
	}
...
function returnElementTDAddEntity(item){	
	...
	if(dataType == 21){
		elementTD += "<span style=\"width: 100%;\"><Label ondblclick=\"openDialogComponentSetting(this)\"";					
					elementTD += " name=\"" 
					+ item.columnname + "1\" maxlength=\"" 
					+ item.maxlength + "\" >"+((item.labelText != undefined && item.labelText != null && item.labelText.length > 0)?item.columnname:item.columnname)+"</label>"+element+"</span>";
	}
	if(dataType == 2){
		elementTD += "<span style=\"width: 100%;\"><input  ondblclick=\"openDialogComponentSetting(this)\" type=\"text\" ";
					if(item.elementtype == 0){
	...
	if(dataType == 13){
		var elementButton =  "" 
			...
			elementButton += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span><button style=\""+item.style+";width:100%\" type=\"button\" class=\"btn qp-button "+buttonStyle+"\" ondblclick=\"openDialogButtonAreaSetting(this)\" value=\""+item.labelText+"\">"+item.labelText+"</button>"+elementButton+"</span>";
	}
	if(dataType == 20){
		var width = "";
...
function returnElementTDPreview(item, displayGroupType){
	...
	var styleButton = 'style="float: left;'+styleDisplayGroupType+'"';
	var style = 'style="width: '+styleWidth+'; float: left;'+styleDisplayGroupType+'"';
	var styleForRadioCheckbox = 'style="float: left;'+styleDisplayGroupType+'"';
	var buttonWidth = "";
	if(item.width != undefined) {
		buttonWidth += 'width:'+item.width+'';
	}
	...
	// daipv: add new for style1
	style1 = style.slice(0, -1);
	
	var tabIndex = "";
	if (item.tabindex != undefined && item.tabindex != null) {
		tabIndex = ' tabindex="'+item.tabindex+'" ';
	}
	
	if(dataType == 0){
		elementTD += "<span "+style+">" +
				"<div class=\"input-group\" style=\"width:100%;" + itemStyleDiv + "\"><input "+tabIndex+" style=\""+ item.inputStyle + ' '+itemStyleInput+"\" type=\"text\" ";
				if(item.elementtype == 0){
					elementTD += " class=\"form-control ui-autocomplete-input\" ";
				} else {
					elementTD += " class=\"qp-input-autocomplete\" ";
	...
	if(dataType == 1 || dataType == 15 || dataType == 16 || dataType == 18){
		...
		elementTD += "<span "+style+"><input "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+"\" type=\""+dataTypeElement+"\" value=\""+item.defaultvalue+"\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-text\"";
					} else {
						elementTD += "class=\"form-control qp-input-text\"";
	...
	if(dataType == 2){
		elementTD += "<span "+style+"><input "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+" qp-input-integer\" type=\"text\" value=\""+item.defaultvalue+"\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-integer\"";
					} else {
						elementTD += "class=\"form-control qp-input-integer\"";
	...
	if(dataType == 3){
		elementTD += "<span "+style+"><input "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+"\" type=\"text\" value=\""+item.defaultvalue+"\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control\"";
					} else {
						elementTD += "class=\"form-control\"";
	...
	if(dataType == 8){
		elementTD += "<span "+style+"><input "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+" qp-input-currency\" type=\"text\" value=\""+item.defaultvalue+"\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-currency\"";
					} else {
						elementTD += "class=\"form-control qp-input-currency\"";
	...
	if(dataType == 4){
		elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div class=\"input-group date qp-input-datepicker\">"
		elementTD += "<span><input "+tabIndex+" style=\""+ item.inputStyle + ' '+itemStyleInput+"\" type=\"text\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"form-control\"";
						} else {
							elementTD += "class=\"form-control\"";
	...
	if(dataType == 14){
		elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div class=\"input-group date qp-input-datetimepicker-detail\">"
		elementTD += "<span><input "+tabIndex+" style=\""+ item.inputStyle + ' '+itemStyleInput+"\" type=\"text\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"form-control\"";
						} else {
	...
	if(dataType == 9){
		elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div class=\"input-group date qp-input-timepicker\">"
			elementTD += "<span><input "+tabIndex+" style=\""+ item.inputStyle + ' '+itemStyleInput+"\" type=\"text\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"form-control\"";
						} else {
	...
	if(dataType == 5){
		if(item.msgvalue != undefined && item.msgvalue.length > 0){
			var msgLabelArr = item.msglabel.split("�");
			var msgValArr = item.msgvalue.split("�");
			elementTD += "<span "+styleForRadioCheckbox+">";
			$(msgValArr).each(function(j){
				elementTD += "<input "+tabIndex+"  type=\"radio\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"qp-input-radio qp-input-radio-margin\"";
				} else {
					elementTD += "class=\"qp-input-radio qp-input-radio-margin\"";
				...
				if(item.msglabel == undefined || item.msglabel.length == 0  || (msgLabelArr[j] == null || msgLabelArr[j] == 'null')){
					...
					elementTD += " name=\"" + item.columnname + "1\" "+checked+" /><label style=\""+ item.inputStyle + ' '+item.style+"\" for=\""+item.columnname+"\">"+deEscapeHTMLForView(msgValArr[j])+"</label>";
				} else {					
					elementTD += " name=\"" + item.columnname + "1\" /><label style=\""+ item.inputStyle + ' '+item.style+"\" for=\""+item.columnname+"\">"+deEscapeHTMLForView(msgLabelArr[j])+"</label>";
				}
			});
			...
			elementTD += "<span "+styleForRadioCheckbox+"><input "+tabIndex+" type=\"radio\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"qp-input-radio qp-input-radio-margin\"";
				} else {
					elementTD += "class=\"qp-input-radio qp-input-radio-margin\"";
	...
	if(dataType == 6){
		if(item.msgvalue != undefined && item.msgvalue.length > 0){
			...
			elementTD += "<span "+styleForRadioCheckbox+">";
			$(msgValArr).each(function(j){
				elementTD += "<input "+tabIndex+" type=\"checkbox\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
				} else {
					elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
	...
	if(dataType == 7){
		elementTD += "<span "+style+"><select "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+"\" name=\""+item.columnname+"\" ";
		if(item.elementtype == 0){
			elementTD += "class=\"form-control qp-input-select\"";
		} else {
			elementTD += "class=\"form-control qp-input-select\"";
		...
		if(item.msgvalue != undefined && item.msgvalue.length > 0){
			...
			$(msgValArr).each(function(j){
				if(item.msglabel == undefined || item.msglabel.length == 0  || (msgLabelArr[j] == null || msgLabelArr[j] == 'null')){
					...
					elementTD += "<option "+checked+">" + deEscapeHTMLForView(msgValArr[j]) + "</option>";
				} else {
					elementTD += "<option>" + deEscapeHTMLForView(msgLabelArr[j]) + "</option>";
				}
			});
		} else {
			elementTD += "<option>"+dbMsgSource['sc.screendesign.0137']+"1</option><option>"+dbMsgSource['sc.screendesign.0137']+"2</option>";
	...
	if(dataType == 10){
		elementTD += "<span "+style+"><textarea "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+"\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-textarea\"";
					} else {
						elementTD += "class=\"form-control qp-input-textarea\"";
	...
	if(dataType == 11){		
		var elementButton =  "" 
			...
			var hoverStyle = "";
			if (item.hoverStyle != undefined && item.hoverStyle != null && item.hoverStyle.length > 0) {
				hoverStyle = "onmouseover=\"this.setAttribute('style', '"+item.hoverStyle+"')\" onmouseout=\"this.setAttribute('style', '"+item.style+"')\"";
			}
			
		elementTD += "<span "+style+"><a "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+"\" "+hoverStyle+" href=\"javascript:\">"+item.labelText+"</a>"+elementButton+"</span>";
	}
	var width = ""
	if(item.width != undefined) {
		width = item.width;
	...
	if(dataType == 12){
		elementTD += "<span "+style+"><input "+tabIndex+" type=\"file\" name=\"" + item.columnname + "1\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"qp-input-file\"";
					} else {
						elementTD += "class=\"qp-input-file\"";
	...
	if(dataType == 13){
		var elementButton =  "" 
			...
			if (item.icon != undefined && item.showLabel != undefined && item.icon.length > 0) {
				...
				elementTD += "<span "+styleButton+"><button "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+";width:100%\" type=\"button\" class=\"btn qp-button "+buttonStyle+"\" value=\""+item.labelText+"\">"+buttonContent+"</button>"+elementButton+"</span>";
			} else {
				elementTD += "<span "+styleButton+"><button "+tabIndex+" style=\""+ item.inputStyle + ' '+item.style+";width:100%\" type=\"button\" class=\"btn qp-button "+buttonStyle+"\" value=\""+item.labelText+"\">"+item.labelText+"</button>"+elementButton+"</span>";
			}
	}
	if(dataType == 20){
		...
		elementTD += "<span style=\"width: "+width+""+widthunit+"; float:left \"><label style=\"cursor: pointer;width: 100%;"+item.style+"\">"+item.labelText+"</label></span>";
	}	
	return elementTD;
}
...
function returnElementTDListAddEntity(item){	
	...
	if(dataType == 13){
		var elementButton =  "" 
			...
			elementButton += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span><button style=\""+item.style+";width:100%\" type=\"button\" class=\"btn qp-button\" ondblclick=\"openDialogButtonAreaSetting(this)\" value=\""+item.labelText+"\">"+item.labelText+"</button>"+elementButton+"</span>";
	}
	if(dataType == 20){
		if (item.labelText == undefined || item.labelText == null) {
			item["labelText"] = dbMsgSource['sc.screendesign.0050'];
...
function returnElemenActionSection(item, styleArea){
	...
	var element =  "" 
		...
		if (item.width != null && item.width != "" && item.width.length > 0) {
			styleItemWidth = "width:" + item.width + item.widthunit + ";";
		}
		
		if (styleArea != undefined && styleArea != null && styleArea.length > 0) {
			style = "style='"+styleArea+ " " +styleItemWidth+"'";
		}/*else {
	...
	if(dataType == 0){
		elementTD += "<span "+style+" >" +
				"<div ondblclick=\"openDialogAutocompleteSettingSection(this)\" class=\"input-group\" style=\"width:100%;"+ item.inputStyle + ' '+itemStyleDiv+"\"><input "+tabIndex+" style=\""+itemStyleInput+"\" type=\"text\" ";
				if(item.elementtype == 0){
					elementTD += " class=\"form-control ui-autocomplete-input\" ";
				} else {
					elementTD += " class=\"qp-input-autocomplete\" ";
	...
	if(dataType == 1 || dataType == 15 || dataType == 16 || dataType == 18){
		...
		elementTD += "<span  "+style+" ><input "+tabIndex+" style=\""+item.style+"\" ondblclick=\"openDialogComponentSettingSection(this)\" type=\""+dataTypeElement+"\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-text\"";
					} else {
						elementTD += "class=\"form-control qp-input-text\"";
	...
	if(dataType == 21){
		...
		elementTD += "<span  "+style+" ><Label style=\""+item.style+"\" ondblclick=\"openDialogComponentSettingSection(this)\"";					
					elementTD += " name=\"" 
					+ item.columnname + "1\" maxlength=\"" 
					+ item.maxlength + "\" >"+((item.labelText != undefined && item.labelText != null && item.labelText.length > 0) ? item.columnname : item.columnname)+"&nbsp"+required+"</label>"+element+"</span>";
	}
	if(dataType == 2){
		elementTD += "<span  "+style+" ><input "+tabIndex+" style=\""+item.style+" qp-input-integer\" ondblclick=\"openDialogComponentSettingSection(this)\" type=\"text\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-integer\"";
					} else {
						elementTD += "class=\"form-control qp-input-integer\"";
	...
	if(dataType == 3){
		elementTD += "<span  "+style+" ><input "+tabIndex+" style=\""+item.style+"\" ondblclick=\"openDialogComponentSettingSection(this)\" type=\"text\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control\"";
					} else {
						elementTD += "class=\"form-control\"";
	...
	if(dataType == 8){
		elementTD += "<span  "+style+" ><input "+tabIndex+" style=\""+item.style+" qp-input-currency\" ondblclick=\"openDialogComponentSettingSection(this)\" type=\"text\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-currency\"";
					} else {
						elementTD += "class=\"form-control qp-input-currency\"";
	...
	if(dataType == 4){
		elementTD += "<span  "+style+"><span><div class=\"input-group date\" style=\"" +itemStyleDiv+ "\">"
		elementTD += "<input "+tabIndex+" style=\""+itemStyleInput+"\" ondblclick=\"openDialogComponentSettingSection(this)\" type=\"text\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"form-control\"";
						} else {
							elementTD += "class=\"form-control\"";
	...
	if(dataType == 5){
		if(item.msgvalue != undefined && item.msgvalue.length > 0){
			...
			$(msgValArr).each(function(j){
				elementTD += "<input "+tabIndex+" type=\"radio\" value=\""+msgValArr[j]+"\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"qp-input-radio qp-input-radio-margin\"";
				} else {
					elementTD += "class=\"qp-input-radio qp-input-radio-margin\"";
	...
	if(dataType == 6){
		...
		if(item.msgvalue != undefined && item.msgvalue.length > 0){
			...
			elementTD += "<span "+style+" ondblclick=\"this.checked = !this.checked;openDialogComponentListSettingSection(this)\">";
			$(msgValArr).each(function(j){
				elementTD += "<input "+tabIndex+" type=\"checkbox\" value=\""+msgValArr[j]+"\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
				} else {
					elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
	...
	if(dataType == 7){
		...
		elementTD += "<span "+style+" ><select "+tabIndex+" style=\""+item.style+";width:100%\" ondblclick=\"openDialogComponentListSettingSection(this)\" name=\""+item.columnname+"\" ";
		if(item.elementtype == 0){
			elementTD += "class=\"form-control qp-input-select\"";
		} else {
			elementTD += "class=\"form-control qp-input-select\"";
		...
		if(msgvalue != undefined && msgvalue.length > 0){
			var msgLabelArr = msglabel.split("�");
			var msgValArr = msgvalue.split("�");
			$(msgValArr).each(function(j){
				if(msglabel == undefined || msglabel.length == 0  || (msgLabelArr[j] == null || msgLabelArr[j] == 'null')){
					elementTD += "<option value=\""+msgValArr[j]+"\">" + deEscapeHTMLForView(msgValArr[j]) + "</option>";
				} else {
					elementTD += "<option value=\""+msgValArr[j]+"\">" + deEscapeHTMLForView(msgLabelArr[j]) + "</option>";
				}
	...
	if(dataType == 10){
		elementTD += "<span "+style+" ><textarea "+tabIndex+" style=\""+item.style+"\" ondblclick=\"openDialogComponentSettingSection(this)\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-textarea\"";
					} else {
						elementTD += "class=\"form-control qp-input-textarea\"";
	...
	if(dataType == 11){		
		var elementButton =  "" 
			...
			elementButton += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span  "+style+" ><a style=\""+item.style+"\" "+hoverStyle+" ondblclick=\"openDialogLinkAreaSettingSection(this);\" href=\"javascript:\">"+item.labelText+"</a>"+elementButton+"</span>";
	}
	var styleLink = "";
	if (styleArea != undefined && styleArea != null && styleArea.length > 0) {
		styleLink = "style=\""+styleArea+"\"";
	}
	if(dataType == 22){
		var localCodelist = "";
		var datasourcetype = "";
		...
		var elementButton =  "" 
			...
			elementButton += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span "+styleLink+"><a "+tabIndex+" style=\""+ item.inputStyle + ' ' +item.style+"\" "+hoverStyle+" ondblclick=\"openDialogLinkAreaSettingSection(this);\" href=\"javascript:\">"+item.columnname+"</a>"+elementButton+"</span>";
	}
	...
	if(dataType == 12){
		elementTD += "<span  "+style+" ><input "+tabIndex+" onclick=\"event.stopPropagation();return false;\" ondblclick=\"event.stopPropagation();openDialogComponentSettingSection(this);return false;\" type=\"file\" name=\"" + item.columnname + "1\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"qp-input-file\"";
					} else {
						elementTD += "class=\"qp-input-file\"";
	...
	if(dataType == 13){
		var elementButton =  "" 
			...
			if (item.icon != undefined && item.showLabel != undefined && item.icon.length > 0) {
				...
				elementTD += "<span "+styleButton+"><button "+tabIndex+" style=\""+item.style+";width: 100%\" type=\"button\" class=\"btn qp-button "+buttonStyle+"\" ondblclick=\"openDialogButtonAreaSettingSection(this)\" value=\""+item.labelText+"\">"+buttonContent+"</button>"+elementButton+"</span>";
			} else {
				elementTD += "<span "+styleButton+"><button "+tabIndex+" style=\""+item.style+";width: 100%\" type=\"button\" class=\"btn qp-button "+buttonStyle+"\" ondblclick=\"openDialogButtonAreaSettingSection(this)\" value=\""+item.labelText+"\">"+item.labelText+"</button>"+elementButton+"</span>";
			}
	}
	if(dataType == 20){
		var widthLabel = '';
		...
		elementTD += "<span  style=\""+styleArea+ " " +widthLabel+ "\" ><label style=\"" +float+ " " +item.style+ "\" ondblclick=\"openDialogLableSettingSection(this)\" style=\"cursor: pointer;float: right;\">"+label;
		var required="";
		if(item.mandatory == "true"){
			required += "<label class=\"qp-required-field\">(*)</label>";
		}
...
function returnElement(div){	
	...
	var element =  "" 
		...
		+ "\",\"linklabel\":\""
		+ "\",\"toscreenid\":\""
		+ "\",\"baseType\":\""+checkUndefined($(div).attr('baseType'))
		+ "\",\"codelistCode\":\""+codelistCode
		+ "\",\"codelistText\":\""+codelistText
		+ "\",\"isEnable\":\""+checkUndefined($(div).attr('isEnable'))
		+ "\",\"localCodelist\":\""+localCodelist
		+ "\",\"isSupportOptionValue\":\""+ isSupportOptionValue
		+ "\",\"maxlength\":\""+maxlength
		+ "\",\"dialogAutocompleteCode\":\""+checkUndefined($(div).attr('dialogAutocompleteCode'))
		+ "\",\"dialogAutocompleteText\":\""+checkUndefined($(div).attr('dialogAutocompleteText'))
		+ "\",\"dialogOnChangeEvent\":\""+checkUndefined($(div).attr('dialogOnChangeEvent'))
		+ "\",\"width\":\""+tablewidth
		+ "\",\"widthunit\":\""+checkUndefined($(div).attr('widthunit'))
		+ "\",\"baseType\":\""+checkUndefined($(div).attr('baseType'))
		+ "\",\"dialogOnSelectEvent\":\""+checkUndefined($(div).attr('dialogOnSelectEvent'))
		+ "\",\"mandatory\":\""+checkUndefined($(div).attr('mandatory'))
		+ "\",\"outputBeanId\":\""+checkUndefined($(div).attr('outputBeanId'))
		+ "\",\"tabindex\":\""+tableindex
		+ "\",\"physicalmaxlength\":\""+physicalmaxlength
		+ "\",\"datasourcetype\":\""+datasourcetype
		+ "\",\"sqldesignid\":\""+checkUndefined($(div).attr('sqldesignid'))
		+ "\",\"sqldesignidtext\":\""+checkUndefined($(div).attr('sqldesignidtext'))
		+ "\",\"optionlabel\":\""+checkUndefined($(div).attr('optionlabel'))
		+ "\",\"optionlabeltext\":\""+checkUndefined($(div).attr('optionlabeltext'))
		+ "\",\"optionvalue\":\""+checkUndefined($(div).attr('optionvalue'))
		+ "\",\"optionvaluetext\":\""+checkUndefined($(div).attr('optionvaluetext'))
		+ "\",\"screenItemId\":\""+checkUndefined($(div).attr('screenItemId'))
		+ "\",\"style\":\""+""
		+ "\",\"hoverStyle\":\""+checkUndefined($(div).attr('hoverStyle'))
		+ "\",\"icon\":\""+checkUndefined($(div).attr('icon'))
		+ "\",\"showLabel\":\""+checkUndefined($(div).attr('showLabel'))
		+ "\",\"enablePassword\":\""+checkUndefined($(div).attr('enablePassword'))
		+ "\",\"allowAnyInput\":\""+checkUndefined($(div).attr('allowAnyInput'))
		+ "\",\"buttonStyle\":\""+checkUndefined($(div).attr('buttonStyle'))
		+ "\",\"navigateToBlogic\":\""+checkUndefined($(div).attr('navigateToBlogic'))
		+ "\",\"navigateToBlogicText\":\""+checkUndefined($(div).attr('navigateToBlogicText'))
		+ "\",\"messageConfirm\":\""+checkUndefined($(div).attr('messageConfirm'))
		+ "\",\"messageConfirmText\":\""+checkUndefined($(div).attr('messageConfirmText'))
		+ "\",\"enableConfirm\":\""+checkUndefined($(div).attr('enableConfirm'))
		+ "\",\"messageConfirmCode\":\""+checkUndefined($(div).attr('messageConfirmCode'))
		+ "\",\"messageLevel\":\""+messageLevel
		+ "\",\"messageLevelText\":\""+messageLevelText
	...
	if(dataType == 21){
		elementTD += "<span style=\"width: 100%;\"><Label ondblclick=\"openDialogComponentSetting(this)\"";					
					elementTD += " name=\"" 
					+ columnname + "\" maxlength=\"" 
					+ $(div).attr('maxlength') + "\" >"+$(div).attr('labelText')+"</label>"+element+"</span>";
	}
	if(dataType == 2){
		elementTD += "<span style=\"width: 100%;\"><input  ondblclick=\"openDialogComponentSetting(this)\" type=\"text\" ";
					if($(div).attr('elementtype') == 0){
	...
	if(dataType == 6){
		if($(div).attr('msgvalue') != undefined && $(div).attr('msgvalue').length > 0){
			...
			if ($(div).attr('physicaldatatype') == undefined || $(div).attr('physicaldatatype') == '' || $(div).attr('physicaldatatype') != 8) {
				elementTD += "<input type=\"checkbox\" ";
				if($(div).attr('elementtype') == 0){
					elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
				} else {
			...
			elementTD += " name=\"" + $(div).attr('columnname') + "\" />";
			if ($(div).attr('physicaldatatype') == undefined || $(div).attr('physicaldatatype') == '' || $(div).attr('physicaldatatype') != 8) {
				elementTD += "<label for=\""+$(div).attr('columnname')+"\">"+dbMsgSource['sc.screendesign.0137']+"2</label>";
			}
			elementTD += ""+element+"</span>";
		}
	}
	...
	if(dataType == 11){
		var elementButton =  "" 
			...
			+ "\",\"maxlength\":\""+maxlength
			+ "\",\"labelText\":\""+$(div).attr("labeltext")
			+ "\",\"navigateToBlogic\":\""+$(div).attr("navigateToBlogic")
			+ "\",\"navigateToBlogicText\":\""+$(div).attr("navigateToBlogicText")
			+ "\",\"messageLevel\":\""+messageLevel
			+ "\",\"messageLevelText\":\""+messageLevelText
			+ "\",\"physicalmaxlength\":\""+physicalmaxlength;
	...
	if(dataType == 23){
		...
		elementLabel += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span span style=\"width: 100%;\"><label class=\"qp-item-custom\" ondblclick=\"openDialogCustomSetting(this, true)\" style=\"cursor: pointer;\">"+$(div).attr('labelText')+"</label>"+elementLabel+"</span>";
	}	
...
function returnElementTDSearch(div){
	...
	if(dataType == 23){
		...
		elementLabel += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span style=\"width: 100%;\"><label class=\"qp-item-custom\" ondblclick=\"openDialogCustomSetting(this, true)\" style=\"cursor: pointer;\">"+$(div).attr('labelText')+"</label>"+elementLabel+"</span>";
	}	
	...
	var tablewidthunit = "";
	if($(div).attr('widthunit') != undefined){
		tablewidthunit = $(div).attr('widthunit');
	} else {
		tablewidthunit = "%";
	}
	if(dataType == 13){
	...
	if(dataType == 20){
		...
		elementLabel += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
		elementTD += "<span style=\"width: 100%;\"><label ondblclick=\"openDialogLableSetting(this, true)\" style=\"cursor: pointer;\">"+$(div).attr('label')+"</label>"+elementLabel+"</span>";
	}
	elementTD += "</td>";
	return elementTD;
}
...
function returnElementSearch(div){
		...
		var data = "";
		if($(div).attr("data") != undefined) {
			data = convertToJson($(div).attr("data"));
		}
		var columnname = "";
		...
		//Start: Use for drap column is text
		if(labelText == "text" && data.labelText != undefined) {
			labelText = data.labelText;
		}
		if(labelText == "Link" && data.labelText != undefined) {
			labelText = data.labelText;
		}
		//End: Use for drap column is text
		...
		var isSupportOptionValue = "";
		if($(div).attr('isSupportOptionValue') != undefined && $(div).attr('isSupportOptionValue') != "") {
			isSupportOptionValue = checkUndefined($(div).attr('isSupportOptionValue'));
		} else {
			isSupportOptionValue = data.isSupportOptionValue;
		}
		
		var mandatory = "";
		if($(div).attr('mandatory') != undefined && $(div).attr('mandatory') != "") {
			mandatory = checkUndefined($(div).attr('mandatory'));
		} else {
		...
		if(dataType == 21){
			elementTD += "<span style=\"width: 100%;\"><Label ondblclick=\"openDialogComponentSetting(this)\" ";					
						elementTD += " name=\"" 
						+ $(div).attr('columnname') + "\" maxlength=\"" 
						//Use for drap column is text
						+ $(div).attr('maxlength') + "\" >"+labelText+"</label>"+element+"</span>";
		}
		if(dataType == 2){
			elementTD += "<span style=\"width: 100%;\"><div  ondblclick=\"openDialogComponentSetting(this)\" style=\"width: 100%;\">" +
					"<input type=\"text\" ";
		...
		if(dataType == 5){
			if( $(div).attr('msgvalue') != undefined && $(div).attr('msgvalue').length > 0){
				...
				$(msgValArr).each(function(j){
					...
					if($(div).attr('msglabel') != undefined && $(div).attr('msglabel').length == 0){					
						elementTD += " name=\"" + $(div).attr('columnname') + "\" value=\""+msgValArr[j]+"\" /><label for=\""+$(div).attr('columnname')+"\">"+msgValArr[j]+"</label>";
					} else {					
						elementTD += " name=\"" + $(div).attr('columnname') + "\" value=\""+msgLabelArr[j]+"\" /><label for=\""+$(div).attr('columnname')+"\">"+msgLabelArr[j]+"</label>";
					}
				});
				elementTD += ""+element+"</span>";
			} else {
		...
		if(dataType == 22){
			var elementButton =  "" 
				...
				elementButton += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
			elementTD += "<span style=\"width: 100%;\"><a ondblclick=\"openDialogLinkAreaSetting(this);\" href=\"javascript:\">"+labelText+"</a>"+elementButton+"</span>";
		}
		...
		if(dataType == 13){
			...
			elementTD += "<span><button style=\"width: 100%; background-color: " +$(div).attr('common-button-bg-color')+"; color: "+$(div).attr('common-button-text-color')+"\" type=\"button\" class=\"btn qp-button\" ondblclick=\"openDialogButtonAreaSetting(this)\" value=\""+$(div).attr('labelText')+"\">"+$(div).attr('labelText')+"</button>"+elementButton+"</span>";
		}
		if(dataType == 20){
			var elementLabel =  "" 
				+ "<input type=\"hidden\" name=\"formElement\" "
			...
			elementLabel += "\",\"require\":\""+require+"\",\"elementtype\":\"0\"'>";
			elementTD += "<span style=\"width: 100%\"><label style=\"width: 100%\" ondblclick=\"openDialogLableSetting(this, true)\" style=\"cursor: pointer; width:100%\">"+$(div).attr('label')+"</label>"+elementLabel+"</span>";
		}
...
function returnElementTDSearchEntityPreview(item, displayGroupType){
	...
	itemStyleInput = itemStyleInput.slice(0, -1);
		...
		if(dataType == 0){
			elementTD += "<span "+style+">" +
					"<div class=\"input-group\" style=\"width:100%;" + itemStyleDiv + "\"><input "+tabIndex+" style=\""+itemStyleInput+"\" type=\"text\" ";
					if(item.elementtype == 0){
						elementTD += " class=\"form-control ui-autocomplete-input\" ";
					} else {
						elementTD += " class=\"qp-input-autocomplete\" ";
		...
		if(dataType == 1 || dataType == 15 || dataType == 16 || dataType == 18){
			...
			elementTD += "<span "+style+"><input "+tabIndex+" style=\""+item.style+"\" type=\""+dataTypeElement+"\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"form-control qp-input-text\"";
						} else {
							elementTD += "class=\"form-control qp-input-text-detail\"";
		...
		if(dataType == 21){
			elementTD += "<span "+style+"><Label style=\""+item.style+"\" ";					
						elementTD += " name=\"" 
						+ item.columnname + "1\" maxlength=\"" 
		...
		if(dataType == 2){
			if (displayFromTo == "0" && areaCustomType == "0") {
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><input "+tabIndex+" style=\""+itemStyleInput+" qp-input-integer\" type=\"text\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"form-control qp-input-integer\"";
				} else {
					elementTD += "class=\"com-input-integer-detail\"";
				...
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div  style=\"width: 100%;\">" +
				"<input "+tabIndex+" style=\""+itemStyleInput+" qp-input-integer\" type=\"text\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-from pull-left qp-input-integer\"";
					} else {
						elementTD += "class=\"com-input-integer-detail\"";
		...
		if(dataType == 3){
			if (displayFromTo == "0" && areaCustomType == "0") {
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><input "+tabIndex+" style=\""+itemStyleInput+" qp-input-float\" type=\"text\" ";
				if(item.elementtype == 0){
					elementTD += "class=\"form-control qp-input-float\"";
				} else {
					elementTD += "class=\"com-input-float-detail\"";
				...
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div  style=\"width: 100%;\">" +
							"<input "+tabIndex+" style=\""+itemStyleInput+"\" type=\"text\" ";
							if(item.elementtype == 0){
								elementTD += "class=\"form-control qp-input-from pull-left qp-input-float\"";
							} else {
								elementTD += "class=\"com-input-float-detail\"";
		...
		if(dataType == 8){
			if (displayFromTo == "0" && areaCustomType == "0") {
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><input "+tabIndex+" style=\""+itemStyleInput+" qp-input-currency\" type=\"text\" ";
							if(item.elementtype == 0){
								elementTD += "class=\"form-control qp-input-currency\"";
							} else {
								elementTD += "class=\"com-input-currency-detail\"";
				...
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div  style=\"width: 100%;\">" +
				"<input "+tabIndex+" style=\""+itemStyleInput+" qp-input-currency\" type=\"text\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"form-control qp-input-from pull-left qp-input-currency\"";
					} else {
						elementTD += "class=\"com-input-currency-detail\"";
		...
		if(dataType == 4){
			if (displayFromTo == "0" && areaCustomType == "0") {
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div>" +
								"<div class=\"input-group date\">"
				elementTD += "<input "+tabIndex+" style=\""+ itemStyleInput+"\" type=\"text\" ";
								if(item.elementtype == 0){
									elementTD += "class=\"form-control\"";
								} else {
									elementTD += "class=\"form-control \"";
		...
		if(dataType == 14){
			if (displayFromTo == "0" && areaCustomType == "0") {
				elementTD += "<span "+style1+ ' ' + itemStyleDiv +  '"' +"><div class=\"input-group date qp-input-datetimepicker-detail\">"
				elementTD += "<span><input "+tabIndex+" style=\""+ itemStyleInput +"\" type=\"text\" ";
								if(item.elementtype == 0){
									elementTD += "class=\"form-control\"";
								} else {
									elementTD += "class=\"form-control \"";
		...
		if(dataType == 6){
			if(item.physicaldatatype != 8) {
				if(item.msgvalue.length > 0){
					...
					elementTD += "<span "+styleForRadioCheckbox+" ><input "+tabIndex+" type=\"checkbox\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
						} else {
							elementTD += "class=\"com-input-checkbox-detail\"";
					...
					elementTD += " name=\"" + item.columnname + "1\" />";
					if (item.physicaldatatype == undefined || item.physicaldatatype == '' || item.physicaldatatype != 8) {
						elementTD += "<label style=\""+item.style+"\" for=\""+item.columnname+"\">"+dbMsgSource['sc.screendesign.0137']+"2</label>";
					}
					elementTD += ""+element+"</span>";
				}
			} else {
				if(item.msgvalue.length > 0) { 
					...
					elementTD += "<span "+style+" ><input "+tabIndex+" type=\"checkbox\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
					} else {
						elementTD += "class=\"com-input-checkbox-detail\"";
					}
					elementTD += " name=\"" + item.columnname + "1\" /><label style=\""+item.style+"\" for=\""+item.columnname+"\">true</label>";
					elementTD += "<input "+tabIndex+" type=\"checkbox\" ";
					if(item.elementtype == 0){
						elementTD += "class=\"qp-input-checkbox-margin qp-input-checkbox\"";
					} else {
					...
					elementTD += " name=\"" + item.columnname + "1\" />";
					elementTD += "<label style=\""+item.style+"\" for=\""+item.columnname+"\">false</label>";
					elementTD += ""+element+"</span>";
				}
			}
		}
		if(dataType == 7){
			elementTD += "<span "+style+"><select "+tabIndex+" style=\""+item.style+"\" name=\""+item.columnname+"\" ";
			if(item.elementtype == 0){
				elementTD += "class=\"form-control qp-input-select\"";
			} else {
				elementTD += "class=\"form-control qp-input-select-detail\"";
			...
			if(item.msgvalue.length > 0){
				...
				$(msgValArr).each(function(j){
					if(item.msglabel == undefined || item.msglabel.length == 0  || (msgLabelArr[j] == null || msgLabelArr[j] == 'null')){
						elementTD += "<option>" + deEscapeHTMLForView(msgValArr[j]) + "</option>";
					} else {
						elementTD += "<option>" + deEscapeHTMLForView(msgLabelArr[j]) + "</option>";
					}
				});
		...
		if(dataType == 10){
			elementTD += "<span "+style+"><textarea "+tabIndex+" style=\""+item.style+"\" ";
						if(item.elementtype == 0){
							elementTD += "class=\"form-control qp-input-textarea\"";
						} else {
							elementTD += "class=\"form-control qp-input-textarea-detail\"";
		...
		if(dataType == 11){	
			...
			elementTD += "<span "+style+"><a "+tabIndex+" style=\""+item.style+"\" "+hoverStyle+" href=\"javascript:\">"+item.labelText+"</a>"+elementButton+"</span>";
		}
		var width = ""
		if(item.width != undefined) {
			width = item.width;
	...
	/*	var width = ""
		...
		if(dataType == 13){
			...
			if (item.icon != undefined && item.showLabel != undefined && item.icon.length > 0) {
				...
				elementTD += "<span "+styleButton+"><button "+tabIndex+" style=\""+item.style+";width:100%\" type=\"button\" class=\"btn qp-button\" value=\""+item.labelText+"\">"+buttonContent+"</button>"+elementButton+"</span>";
			} else {
				elementTD += "<span "+styleButton+"><button "+tabIndex+" style=\""+item.style+";width:100%\" type=\"button\" class=\"btn qp-button\" value=\""+item.labelText+"\">"+item.labelText+"</button>"+elementButton+"</span>";
			}
		}
		if(dataType == 20){
			var width = "";
...
function insertAnyElement(ev, ui, obj){
	...
	var sqldesignid = "";
	if($(div).attr('sqldesignid') != undefined && $(div).attr('sqldesignid') != "") {
		sqldesignid = $(div).attr('sqldesignid');
	}
	...
	if ($(div).attr('parameters') == undefined || $(div).attr('parameters') == null || $(div).attr('parameters').length == 0) {
		...
		//Use for drap column
		if($(div).attr('codelisttext') != undefined && $(div).attr('codelisttext') != "") {
			parameters = $(div).attr('parameters');
			datasourcetype = 2;
		} else {
			parameters = $(div).attr('parameters');
	...
	var label = "";
	if($(div).attr('label') != undefined) {
		label = $(div).attr('label');
	}
	...
	if(dataType == 21){
		elementTD += "<label ondblclick=\"openDialogComponentSettingSection(this)\" ";
					elementTD += " name=\"" 
					+ $(div).attr('columnname') + "\" maxlength=\"" 
					+ $(div).attr('maxlength') + "\" >"+/*$(div).attr('columnname')*/  labelText+"</label>"+element+"";
	}
	if(dataType == 2){
		elementTD += "<input  ondblclick=\"openDialogComponentSettingSection(this)\" type=\"text\" ";
					if($(div).attr('elementtype') == 0){
	...
	if(dataType == 20){
		elementTD += "<label ondblclick=\"openDialogLableSettingSection(this)\" style=\"cursor: pointer;\">"+dbMsgSource['sc.screendesign.0050']+"</label>"+element;
	}	
	...
	// daipv: comment for [height] is 30px
//	if (dataType != 10) {
//		style += "height: 30px;";
...
function insertActionElement(ev, ui, obj){
	...
	if(dataType == 21){
		elementTD += "<label ondblclick=\"openDialogComponentSettingSection(this)\" ";					
					elementTD += " name=\"" 
					+ $(div).attr('columnname') + "\" maxlength=\"" 
					+ $(div).attr('maxlength') + "\" >"+labelText+"</label>"+element+"";
	}
	if(dataType == 2){
		elementTD += "<input  ondblclick=\"openDialogComponentSettingSection(this)\" type=\"text\" ";
					if($(div).attr('elementtype') == 0){
...
function addEntity(searchType){
	...
	if ($("[name=tableMapping]").val() == undefined || $("[name=tableMapping]").val() == '' || parseInt($("[name=tableMapping]").val()) == 0) {
		$.qp.showAlertModal(dbMsgSource["sc.screendesign.0165"]);
		return;
	}
	...
	if (data != '' && data != null){
		...
		if($("[name=radioEntityType]:checked").val() == "0" || $("[name=radioEntityType]:checked").val() == "2"){
			...
			var countSearchCondition = 0;
				...
				if (data.length > 0) {
					...
					for (i = 0; i < data.length; i++) {
						//TungHT 25/7 fix code for formatcode
						if(data[i].datatype == "15") {
							data[i].formatcode = "Email";
						}
						if(data[i].datatype == "16") {
							data[i].formatcode = "Phone";
						}
						if(data[i].datatype == "18") {
							data[i].formatcode = "Alphanumeric";
						}
						
						var obj = data[i];//mandatory
						if (obj.itemType == '2') continue;
						if (searchType != null && searchType == 1) {
							var objTemp = obj;
							//TungHT - 20/7
							/*objTemp['mandatory'] = false;*/
							content += "<th index=\""+indexCol+"\">"+returnElementTHAddEntity(objTemp, "false")+"</th>";
						} else {
						...
						if (i == data.length - 1 && j < 2) { 
							...
							content += "<td index=\""+parseInt(++indexCol)+"\"><input type=\"hidden\" name=\"groupDisplayType\"  /><input type=\"hidden\" name=\"groupTotalElement\"  /><input type=\"hidden\" name=\"enableGroup\"  /><div class=\"dropComponent\">&nbsp;<input type=\"hidden\" name=\"formElement\" value=\"\"></div></td>";												
							j++;
						}
					...
					$.qp.showAlertModal(dbMsgSource["sc.screendesign.0166"]);
				}
				content += "</tbody></table>" +
			returnAddNewRow() + "</div></div>";
				//if(countSearchCondition == 0){
					...
					if(maxIndex > 0) {
						$(".form-area-content:last").find("table:last").find("input[type='radio']").each(function(){
							$(this).attr("name", $(this).attr("name") + maxIndex);
						});
					}
					for (i = 0; i < data.length; i++) {
						var obj = data[i];//mandatory
						...
						hiddenField += obj.columnname + "π" + obj.label + "π" + obj.labelText + "ππ" + obj.physicaldatatype + "�";
					}
					hiddenField = hiddenField.substring(0, hiddenField.length - 1);
					
					$(".form-area-content:last").find(".areaContent:last").find("input[name=formElementHidden]").val(hiddenField);
				...
				//} else {
					//$.qp.showAlertModal(dbMsgSource["sc.screendesign.0167"]);
				//}
		} else {
			...
			if (data.length > 0) {
				...
				var content = returnTableListSetting(selectScreenType, tableName, name); 
				...
				width = parseInt(100/countHidden);
				for (i = 0; i < data.length; i++) {
					var obj = data[i];
					if (obj.itemType == '2') continue;
				...
				for (i = 0; i < data.length; i++) {
					...
					if (selectScreenType == "3") {
						obj.datatype= "21";
						content += "<td index=\""+i+"\"><span style=\"float: left; cursor: move;\" class=\"qp-glyphicon  glyphicon glyphicon-screenshot\" title=\""+fcomMsgSource['sc.sys.0015']+"\"></span><input type=\"hidden\" name=\"groupDisplayType\"  /><input type=\"hidden\" name=\"enableGroup\"  /><input type=\"hidden\" name=\"groupTotalElement\"  />"+returnElementTDLoad(obj)+"</td>";
					} else {
						content += "<td index=\""+i+"\"><span style=\"float: left; cursor: move;\" class=\"qp-glyphicon  glyphicon glyphicon-screenshot\" title=\""+fcomMsgSource['sc.sys.0015']+"\"></span><input type=\"hidden\" name=\"groupDisplayType\"  /><input type=\"hidden\" name=\"enableGroup\"  /><input type=\"hidden\" name=\"groupTotalElement\"  />"+returnElementTDLoad(obj)+"</td>";
					}
				}
	...
	var tableId = $(".form-area-content:last").find("table:last").attr("id");
	...
	$(".form-area-content:last").find("table:last").find(".glyphicon-screenshot").draggable({
		containment: '#' + tableId,
		stack: '#' + tableId,
		revert: function(event, ui) {
				if ($(this).data("uiDraggable") != undefined && $(this).data("uiDraggable").originalPosition != undefined) {
	...
	if($("[name=radioEntityType]:checked").val() == "0"){
		$(".form-area-content:last").find("table:last").find("th").find(".glyphicon-screenshot").droppable({
			accept: "#" + tableId + " tr:not(:first) th .glyphicon-screenshot",
			activeClass: "state-droppable",
			drop: function(event, ui) {
				replaceElement(event, ui, $(this));
			}
		...
		$(".form-area-content:last").find("table:last tbody").find("td").find(".glyphicon-screenshot").droppable({
			accept: "#" + tableId + " tr:not(:first) td .glyphicon-screenshot",
			activeClass: "state-droppable",
			drop: function(event, ui) {
				replaceElement(event, ui, $(this));
			}
	...
	//default group
	$(".form-area-content:last").find("table:last tbody").find("td").droppable({
		accept: "#srcgenElements div, #srcgenAction div, #newDragElementTd div",
		activeClass: "state-droppable",
		drop: function(event, ui) {					
			insertComponent(event, ui, $(this));			
	...
	$(".form-area-content:last").find("table:last tbody").find("td").droppable( "option", "disabled", false );
	$(".form-area-content:last").find("table:last tbody").find("td").find("input[name='enableGroup']").val(true);
	$(".form-area-content:last").find("table:last tbody").find("td").find("input[name='groupDisplayType']").val(1);
...
function addRowTBJS(obj) {		
	...
	var rowCount = parseInt($(table).find("tr:last").attr("index")) + 1;
	 ...
	 if (rowCount >=  TABLE_SETTING.rowLimit) {
		$.qp.showAlertModal(dbMsgSource["sc.screendesign.0168"] + " " + TABLE_SETTING.rowLimit);
		 return;
	 }
...
function merge(td, mergeNew, mergeOld) {
	...
	if (!isValidCol) {
		if (maxCol > 0) {
			$.qp.showAlertModal(dbMsgSource['sc.screendesign.0126'].replace("{0}", maxCol));
			return false;
		}		
	}
	var isValidRow = true;
	...
	if (!isValidRow) {		
		if (maxRow > 0) {
			$.qp.showAlertModal(dbMsgSource['sc.screendesign.0125'].replace("{0}", maxRow));
			return false;
		}
	}
	...
	if (!isValid) {		
		$.qp.showAlertModal(dbMsgSource["sc.screendesign.0169"]);
		return false;			
	}
...
function  removeRowTBJS(obj) {
	...
	if (!isValid) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0170']);
		return false;
	}
...
function addColumnJS(obj) {	
	...
	if (!checkColRowSpanAddCol($(obj))) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0172']);
		return;
	}
	 ...
	 if (($(tr).children().length-2) * 2>=  TABLE_SETTING.columnLimit) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0171'] + " " + TABLE_SETTING.columnLimit);
		 return;
	 }
	 var table = $(obj).closest("table");
	...
	*/
	...
	var headerStyle = $(table).closest(".areaContent").find("input[name=headerStyle]").val();
	var inputStyle = $(table).closest(".areaContent").find("input[name=inputStyle]").val();
	if(headerStyle != undefined && headerStyle != "" && headerStyle != null) {
		$(table).find("tr").find("th").attr("style", headerStyle);
	}
	if(inputStyle != undefined && inputStyle != "" && inputStyle != null) {
...
function  removeColumnJS(obj) {
	...
	if (!checkColRowSpanAddRemoveCol($(obj))) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0173']);
		return;
	}
...
function addColumnTBJS(obj) {	
	if (!checkColRowSpanAddColList($(obj))) {
	 ...
	 var headerLabelRow = $(obj).closest("div").prev("div").find("input[name='formHeaderLabelRow']").val();
	 var index = $(obj).closest("td").index();
	 var tr = $(obj).closest("tr");
	 var table = $(obj).closest("table");
	...
	*/
	...
	var index = $(obj).closest("td").index();
	var string = $(obj).closest("table").parent("div").prev().find("input[name='formScreenAreaSortValue']").val();
	var data = convertToJson(string);
	var modal = $("#modal-table-list-setting");
	
	var arrData = [];
	var i = 0;
	var screenId = $(table).closest("#screenDesignForm").find("input[name='screenId']").val();
	var currentLengthOfTH = $(obj).closest("div").find("table thead tr:first th").length;
	if(headerLabelRow == 1 || headerLabelRow == "") {
		while(data.areaSorts != undefined  && data.areaSorts != null && i < data.areaSorts.length) {
			if(headerLabelRow == 1 || headerLabelRow == "") {
				if (i == parseInt(index)+1) {
					arrData.push({
						screenAreaCode : data.areaSorts[i].screenAreaCode,
						screenItemCode : "",
						sqlColumnCode : "",
						screenId : screenId,
						screenAreaId : ""
			...
			arrData.push(data.areaSorts[i]);
			i++;
		}
	} else {
		if (data.areaSorts != undefined  && data.areaSorts != null && data.areaSorts.length > 0) {
			var increase = 0;
			for(var i = 0; i < headerLabelRow; i++) {
				for(var j = 0; j < currentLengthOfTH - 1; j++) {
					if (j == parseInt(index)+1) {
						arrData.push({
							screenAreaCode : data.areaSorts[j].screenAreaCode,
							screenItemCode : "",
							sqlColumnCode : "",
							screenId : screenId,
							screenAreaId : ""
					...
					arrData.push(data.areaSorts[increase]);
					increase++;
				}
			}
		}
	...
	data.areaSorts = arrData;
	var dataStr = convertToString(data);
	$(table).closest(".areaContent").find("input[name='formScreenAreaSortValue']").val(dataStr);
...
function  removeColumnTBJS(obj) {
  ...
  var string = $(table).closest(".areaContent").find("input[name='formScreenAreaSortValue']").val();
  var data = convertToJson(string);
  var arrData = [];
  var i = 0;
  var screenId = $(table).closest("#screenDesignForm").find("input[name='screenId']").val();
  ...
  if(headerLabelRow == 1 || headerLabelRow == "") { 
	  if(data.areaSorts != undefined) {
		  while(i < data.areaSorts.length) {
			  if(i != index) {
				  arrData.push(data.areaSorts[i]);
			  }
			  i++;
	  ...
	  var increase = 0;
		for(var i = 0; i < headerLabelRow; i++) {
			for(var j = 0; j < currentLengthOfTH; j++) {
				if (j != index) {
					arrData.push(data.areaSorts[increase]);
				}
				increase++;
...
function deleteTable(obj){
	if ($(obj).closest(".area-tab") != undefined && $(obj).closest(".area-tab").length > 0) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0379']);
		return false;
	} else {
...
function setBaseType(data) {
	...
	case '0': case '5': case '6': case '7':
		...
		baseType += "<option value=\"2\">Char</option>";
		...
		baseType += "<option value=\"13\">Serial</option>";
	...
	case '3':
		...
		baseType += "<option value=\"4\">Numeric</option>";
		baseType += "<option value=\"13\">Serial</option>";
		baseType += "<option value=\"14\">Bigserial</option>";
		baseType += "</select>";
		break;
...
function changeTypeCodeList(thiz){
	...
	var trCodelistSys = $(thiz).parents("table").find("tr[class=codelist-system]").get(0);
	var valueOfCodelistSys = $(trCodelistSys).find("input").val();
	
	if(value == 1){
		$(thiz).parents("table").find("tr[class=codelist-system]").show();
	}else if(value == 3){
		if (tbody.children().length == 0 && (valueOfCodelistSys == undefined || valueOfCodelistSys == "")) {
			$(modal).find("#dialog-component-list-setting-tbl-options").find("tbody").append(tdContent);
		}
		if (tbodyDisplaySection.children().length == 0 && (valueOfCodelistSys == undefined || valueOfCodelistSys == "")) {
...
function swapColumn(ev, ui, obj){
	...
	if (checkSwapHeader(table, indexFrom)) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0192']);
		return;
	}
...
function insertForm(ev, ui, obj){
	...
	var setting = "<span class=\"form-setting\">" +
						"<br>" +
						"<a href=\"javascript:\" onclick=\"openFormSetting(this)\" class=\"ui-state-default qp-glyphicon-cog glyphicon glyphicon-cog\" title=\"Table properties setting\"></a>&nbsp;" +
...
function saveOutputSetting($modal, data, obj) {
	
	var value = $modal.find('.mapping-bean').find('input[name=radChooseBean]:checked').val();
	var level = $modal.find('.mapping-bean').find('input[name=radChooseBean]:checked').closest('tr').attr('level');
	...
	var $table = $(obj).closest('table');
	$table.find('tbody').find('input[name=formElement]').each(function(){
		var dataElement = convertToJson($(this).val());
		
		if (dataElement.outputBeanLevel != undefined && dataElement.outputBeanLevel != null && dataElement.outputBeanLevel == level) {
			isExists = true;
			return false;
		}
	});
	...
	if (isExists) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0285']);
		return false;
	}
	...
	$table.find('tbody').find('input[name=formElement]').each(function(){
		...
		if (dataElement.outputBeanLevel != undefined && dataElement.outputBeanLevel != null) {
			levelParent = dataElement.outputBeanLevel;
			return false;
		}
	});
	
	levelParent = levelParent.substring(0, levelParent.lastIndexOf('.'));
	var currentLevelParent = level.substring(0, level.lastIndexOf('.')); 
	
	if (levelParent != '' && levelParent != currentLevelParent) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0286']);
		return false;
	}
	
	if (data.outputBeanId != undefined) {
		data.outputBeanId = value;
	} else {		
		data["outputBeanId"] = value;
	}
	
	if (data.outputBeanLevel != undefined) {
		data.outputBeanLevel = level;
	} else {		
		data["outputBeanLevel"] = level;
	}
...
function loadScreenItemCode(obj) {
	var results = [];
	var $forms = $('.form-area-content');
	var beanIsArray = obj.arg03; 
	
	$forms.each(function(formIndex) {
		var $form = $(this);
		var formCode = $form.find('input[name=screenFormFormActionCode]').val();
		results.push({
			"optionLabel" : 'Form: ' + formCode,
			"optionValue" : formCode,
			"level" : formIndex,
			"hasChild": true
		});
		$form.find('.areaContent').each(function(areaIndex){
			...
			if (areaType != 2 && ((obj.arg03 == null || obj.arg03.length == 0 || isArray == beanIsArray) || beanIsArray == "true")) {
				var code = $area.find('input[name=formAreaCode]').val();
				var name = $area.find('input[name=formAreaCaptionText]').val();
				...
				results.push({
					"optionLabel" : 'Area: ' +areaCode,
					"optionValue" : areaCode,
					"level" : formIndex + '.' + areaIndex,
					"hasChild": true
				});
				
				//load hidden
				var hiddenValue = $(this).find('input[name=formElementHidden]').val();
				if(hiddenValue != undefined && hiddenValue.length > 0){
					var arrParameters = hiddenValue.split("�");
					
					$(arrParameters).each(function(i){
						var dataParameter = arrParameters[i].split("π");
						
						results.push({
							"optionLabel" :dataParameter[2],
							"optionValue" : formCode + '.' + areaCode + '.' + dataParameter[0],
							"level" : formIndex + '.' + areaIndex + '.' + -1,
							"hasChild": false
						});
				...
				$area.find('input[name=formElement]').each(function(itemIndex) {
					if ($(this).val() != undefined && $(this).val().length > 0) {
						var data = convertToJson($(this).val());
						if (data.datatype != undefined && data.datatype.length > 0  && data.datatype != 20 && data.datatype != 13 && data.datatype != 11) {
							if (data.displayFromTo != undefined && data.displayFromTo != null && data.displayFromTo == 1) {
								results.push({
									"optionLabel" :data.columnname + "From",
									"optionValue" : formCode + '.' + areaCode + '.' + data.columnname + "From",
									"level" : formIndex + '.' + areaIndex + '.' + itemIndex,
									"hasChild": false
								});
								results.push({
									"optionLabel" :data.columnname + "To",
									"optionValue" : formCode + '.' + areaCode + '.' + data.columnname + "To",
									"level" : formIndex + '.' + areaIndex + '.' + itemIndex,
									"hasChild": false
								});
							} else {
								results.push({
									"optionLabel" :data.columnname,
									"optionValue" : formCode + '.' + areaCode + '.' + data.columnname,
									"level" : formIndex + '.' + areaIndex + '.' + itemIndex,
									"hasChild": false
								});
							}
...
function loadParameter(obj, defaultParameters) {
	
	var table = $(obj).closest('table');
	var modal = $(obj).closest('.modal-dialog').closest('.modal');
	
	if ($(table).find("input[name=isSubmit]").prop("checked")) {
	...
	var isArrayArea = "false";
	var areaType = $($(modal).data('target')).closest('.areaContent').find('input[name=formAreaType]').val();
	
	if (areaType == 1 || areaType == '1') {
		isArrayArea = "true";
	}
	
	var actionType = $(table).find("input[name=actiontype]:checked").val();
	//comment param by screen
	if (false) {
		
		var parameters = $.qp.getString(CONTEXT_PATH + '/screendesign/getScreenParams?screenId=' + value + '&r=' + Math.random());
		parameters = unescape(parameters).replace( /\#\#\?\#\#/g, '�').replace(/&pi;/g, 'π');
		
		if(parameters != undefined && parameters.length > 0){
			var arrParameters = parameters.split("�");
			var $screenItemCode = $('#autocomplete-template').tmpl();
			$screenItemCode.find("input[id='screenItemCodeAutocompleteId']").attr("value",$(table).find('#dialogActionParamSetting').find("input[id='screenItemCodeAutocompleteId']").val());
			$screenItemCode.find("input[name='screenItemCode']").attr("value",$(table).find('#dialogActionParamSetting').find("input[id='screenItemCodeAutocompleteId']").val());
			$(table).find("#dialog-form-parameter-tbl-parameter").find("tbody").empty();
			$(arrParameters).each(function(i){
				if (arrParameters[i] != null && arrParameters[i].length > 0) {
					var dataParameter = arrParameters[i].split("π");
					var dataType = CL_DATATYPE_PARAMETERS[dataParameter[1]];
					var $content = '' +
						'<tr>' +
							'<td>'+(i + 1)+'</td>' +
							'<td>'+dataParameter[0]+'<input type="hidden" name="parameterAttribute" value="'+dataParameter[0]+'" />'+'<input type="hidden" name="parameterAttributeDatatype" value="'+dataType+'" />'+'<input type="hidden" name="parameterAttributeDatatypeId" value="'+dataParameter[1]+'" />'+'</td>' +
							'<td>'+dataType+'</td>' +
							'<td><div name="screenItemCode"></div></td>' +
						'</tr>';
					$(table).find("#dialog-form-parameter-tbl-parameter").find("tbody").append($content);
				}
			});
			
			
			
			$(table).find("#dialog-form-parameter-tbl-parameter").find("tbody").find('div[name=screenItemCode]').replaceWith($screenItemCode);
			$(table).find("#dialog-form-parameter-tbl-parameter").find('tbody tr').each(function(){
				var dataType  = $(this).find('input[name=parameterAttributeDatatype]').val();
				$(this).find('#screenItemCodeAutocompleteId').attr('arg01', dataType);
				$(this).find('#screenItemCodeAutocompleteId').attr('arg03', isArrayArea);
				$.qp.initialCatAutocomplete($(this).find('#screenItemCodeAutocompleteId'));
			});
			
			//set selected
			if (defaultParameters != undefined && defaultParameters != null && defaultParameters.length > 0) {
				var arrParameters = defaultParameters.split("�");
				$(arrParameters).each(function(i) {
					if (arrParameters[i] != null && arrParameters[i].length > 0) {
						var param = arrParameters[i].split("π");
						var paramElement = $(table).find("#dialog-form-parameter-tbl-parameter").find("tbody").find('input[name=parameterAttribute][value="'+param[0]+'"]');
						if (paramElement != undefined && paramElement != null && paramElement.length > 0) {
							$(paramElement).closest('tr').find('input[name=screenItemCodeAutocomplete]').val(getCode(param[1]));
							$(paramElement).closest('tr').find('input[name=screenItemCodeAutocomplete]').attr("value",getCode(param[1]));
							$(paramElement).closest('tr').find('input[name=screenItemCode]').val(param[1]);
						}
					}
				});
			} else {
				$(table).find("#dialog-form-parameter-tbl-parameter").find('tbody tr').each(function(){
					$(this).find('input[name=screenItemCodeAutocomplete]').val("");
					$(this).find('input[name=screenItemCode]').val("");
				});
			}
			
			$(table).find('#dialogActionParamSetting').show();
			$(table).find('#dialog-form-parameter-tbl-parameter').show();
		} else {
			$(table).find('#dialogActionParamSetting').hide();
		...
		var dataString = "";
		try{
			dataString = $.qp.getString(CONTEXT_PATH + '/screendesign/getBusinessDesign?businesslogicid=' + value + '&r=' + Math.random());
			
			var data = convertToJson(dataString);
			var inputBeans = data.lstInputBean;
			var $screenItemCode = $('#autocomplete-template').tmpl();
					
			if (inputBeans != undefined && inputBeans != null && inputBeans.length > 0) {
				
				$(table).find('.mapping-bean').find('tbody').empty();
				for (var i = 0; i < inputBeans.length; i++) {
					...
					var parentIsArray = $(table).find('.mapping-bean').find('tbody tr[beanid='+parentBeanId+']').attr('isobjectarray');
					...
					var content = '<tr beanid="'+inputBeans[i].inputBeanId+'" level="'+inputBeans[i].tableIndex+'" isobjectarray="'+inputBeans[i].arrayFlg+'"  isobjectdatatype="'+inputBeans[i].dataType+'" data-ar-rgroup="'+inputBeans[i].groupId+'">' +
										...
										'<div style="height: 100%;">' +
											...
											'<div style="float:left;height:100%;vertical-align: middle; padding: 6px 7px;">' +
												inputBeans[i].messageStringAutocomplete+
											'</div>' +
										'</div>' +
									'</td>' +
									'<td>' +
						...
						content += '</td>' +
								'</tr>';
					$(table).find('.mapping-bean').find('tbody').append(content);
					if (parentBeanId == null) {
						$(table).find('.mapping-bean').find('tbody tr:eq('+i+')').find('input[name=screenItemCodeAutocomplete]').attr('arg03', isArrayArea);
					} else {
						$(table).find('.mapping-bean').find('tbody tr:eq('+i+')').find('input[name=screenItemCodeAutocomplete]').attr('arg03', isArrayArea);
					}
					...
					if (defaultParameters != undefined && defaultParameters != null && defaultParameters.length > 0) {
						var arrParameters = defaultParameters.split("�");
						$(arrParameters).each(function(j) {
							if (arrParameters[j] != null && arrParameters[j].length > 0) {
								var param = arrParameters[j].split("π");
								
								if (inputBeans[i].inputBeanId == param[0]) {
									$(table).find('.mapping-bean').find('tbody tr:eq('+i+')').find('input[name=screenItemCodeAutocomplete]').val(getCode(param[1]));
									$(table).find('.mapping-bean').find('tbody tr:eq('+i+')').find('input[name=screenItemCode]').val(param[1]);
								}
							}
						});
					} else {
						$(table).find("#blogic-param").find('tbody tr').each(function(){
							$(this).find('input[name=screenItemCodeAutocomplete]').val("");
							$(this).find('input[name=screenItemCode]').val("");
						});
						
						$(table).find('#dialogActionParamSetting').hide();
						$(table).find('#blogic-param').hide();
					}
					
					
					$.qp.initialCatAutocomplete($(table).find('.mapping-bean').find('tbody tr:eq('+i+')').find('input[name=screenItemCodeAutocomplete]'));
				}
...
function saveActionParameter($modal, data) {
	...
	if (false) {
		...
		if (!isValid) {
			$.qp.showAlertModal(errorMessage);
			return false;
		}
		...
		$modal.find('#blogic-param').find('tbody tr').each(function (i){
		...
		if (isExists) {
			$.qp.showAlertModal(existsCode + ' is duplicate');
			return false;
		}
...
function AddActionListComponent(btn) {
	var parent = $(btn).closest('#dialog-component-list-setting-event, #dialog-component-list-setting-section-event');
	var actionType = $(parent).find('#actionType').val();
	
	if (actionType == 0) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0323']);
		$(parent).find('#actionType').focus();
		return;
	}
	...
	if (actionType == 1) {
		...
		$(parent).find('#listEvent').prepend($reloadEvent);
	} else {
		if ($(parent).find('#listEvent').find(".onsubmit-event").length > 0) {
			$.qp.showAlertModal(dbMsgSource['sc.screendesign.0324']);
			return;
		}
		
		var $submitEvent = $('#action-submit-template').tmpl();
		
		$(parent).find('#listEvent').prepend($submitEvent);
	}
	...
	var newEvent = $(parent).find('#listEvent').find('.onchange-event:first'); 
	...
	$(parent).find('#listEvent').sortable({
		  ...
		  items: '.onchange-event,.onsubmit-event',
		  cursor: 'move',
...
function removeEvent(obj){
	if($.qp.confirm(fcomMsgSource['inf.sys.0014'])){
		$(obj).closest('.onchange-event, .onsubmit-event').remove();		
	}
}

function effectAreaTypeChoose(obj) {
	var parent = $(obj).closest('.onchange-event');
	$(parent).find('#effectAreaAutocompleteId').val('');
	$(parent).find('input[name=effectArea]').val('');
	$(parent).find('#effectAreaAutocompleteId').attr('sourcecallback', 'getEffectArea');
	$(parent).find('#effectAreaAutocompleteId').attr('sourcetype', 'local');
	$(parent).find('#effectAreaAutocompleteId').attr('arg01', $(obj).val());
	
	$.qp.initialCatAutocomplete($(parent).find('#effectAreaAutocompleteId'));
}

function checkEffectAreaType(obj) {
	var parent = $(obj).closest('.onchange-event');
	var checked = $(parent).find('input[name^=radEffectAreaType]:checked');
	
	if (checked == undefined || checked == null || checked.length == 0) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0287']);
		$(parent).find('input[name^=radEffectAreaType]')[0].focus();
		return false;
	}
	return true;
}

function getItemCode(name, code) {
	var itemCode = code;
...
function getEffectArea(obj, target) {
	
	var modal = $(target).closest('.modal');
	var targetCode = "";
	if (target != undefined && target != null && modal != undefined && modal != null && modal.length > 0) {
		var current = $(modal).data("target"); 
		targetCode = $(current).closest('.form-area-content').find('input[name=screenFormFormActionCode]').val() + "." 
		+ $(current).closest('.areaContent').find('input[name=formAreaCode]').val();
		if ($(current).next() != undefined && $(current).next() != null && $(current).next().length > 0 && $(current).next().val() != undefined && $(current).next().val() != null && $(current).next().val().length > 0) {
			var currentJson = convertToJson($(current).next().val());
			targetCode += "." + currentJson.columnname;
		}
		
	}
	
	var effectAreaType = obj.arg01;
	var $screen = $('#srcgenScreenAndAction'); 
	var results = [];
	switch(effectAreaType) {
		case '0':
			$screen.find('.form-area-content').each(function(formIndex){
				$form = $(this);
				var formCode = $form.find('input[name=screenFormFormActionCode]').val();
				...
				$form.find('.areaContent').each(function(index){
					var $area = $(this);
					var areaName = $area.find('input[name=formAreaCaptionText]').val();
					var areaCode = $area.find('input[name=formAreaCode]').val();
					
					if ($area.find('input[name=formAreaType]').val() == 0) {
						results.push({
							"optionLabel" : getItemCode(areaName, areaCode),
							"optionValue" : formCode + '.' +getItemCode(areaName, areaCode),
							"level" : formIndex + '.' + index,
							"hasChild": false
						});
					}
				});
		...
		case '1':
			...
			$screen.find('.form-area-content').each(function(formIndex){
				...
				$form.find('.areaContent').each(function(index){
					...
					if ($area.find('input[name=formAreaType]').val() == 1) {
						results.push({
							"optionLabel" : getItemCode(areaName, areaCode),
							"optionValue" : formCode + '.' + getItemCode(areaName, areaCode),
							"level" : formIndex + '.' + index,
					...
					if (areaType != 2) {
						...
						$area.find('input[name=formElement]').each(function(itemIndex) {
							...
							if (targetCode.length > 0) {
								...
								if (targetCode != formCode + '.' + areaCode + '.' + data.columnname && data.datatype != 22 && data.columnname != undefined) {
									if (data.datatype != 20 && data.datatype != 13 && data.datatype != 11) {
		...
		case '3':
			$screen.find('.form-area-content').each(function(formIndex){
				...
				$form.find('.areaContent').each(function(index){
					...
					if ($area.find('input[name=formAreaType]').val() == 3) {
						results.push({
							"optionLabel" : getItemCode(areaName, areaCode),
							"optionValue" : formCode + '.' +getItemCode(areaName, areaCode),
							"level" : formIndex + '.' + index,
...
function validateBLogicNameEventSetting(blogic) {
	
	if ($(blogic).find('input[name=businessLoginAutocomplete]').val() == undefined || $(blogic).find('input[name=businessLoginAutocomplete]').val().length == 0) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0288']);
		$(blogic).find('input[name=businessLoginAutocompleteAutocomplete]').focus();
		return false;
	}
...
function validateEffectAreaEventSetting(blogic) {
	
	if ($(blogic).find('input[name=effectArea]').val() == undefined || $(blogic).find('input[name=effectArea]').val().length == 0) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0289']);
		$(blogic).find('input[name=effectAreaAutocomplete]').focus();
		return false;
	}
...
function getAreaByCode(code) {
	...
	if (arr.length == 1) {
		...
		$screen.find('.form-area-content').each(function(){
			var formCode = $(this).find('input[name=screenFormFormActionCode]').val();
			if (formCode == arr[0]) {
				$element = $(this);
				return false;
			}
			...
			if (formCode == arr[0]) {
				$(this).find('.areaContent').each(function() {
					...
					if (areaCode == arr[1]) {
						...
						$(this).find('input[name=formElement]').each(function() {
							var data = convertToJson($(this).val());
							if (data.datatype != 20 && data.datatype != 13 && data.datatype != 11) {
								
								var itemCode = data.columnname;
								if (itemCode == arr[2]) {
									$element = $(this).closest('.areaContent');
									return false;
								}
							}
						});
...
function saveProperties(modal, data) {
	...
	if ($(modal).find("input[name='labelName']").val().length == 0) {
		$.qp.showAlertModal(dbMsgSource['sc.screendesign.0278']);
		return false;
	}
	...
	if(labelText == undefined || labelText == "" || labelText == null) {
		labelText = $(modal).find("input[name='labelNameAutocomplete']").attr("arg08");
	}
	
	/*var lableTextElement = getMessageLevelText($(modal).find("input[name='labelNameAutocomplete']")) ; 
	if(lableTextElement.val() != undefined){
		labelText =lableTextElement.val();
	}*/
	
	var displayType = $(modal).find("input[name='displayType']:checked").val();
...
function changeSettingDataSource(obj) {
	var parent = $(obj).closest('.panel-body');
	var value = $(obj).val();
	
	switch(value) {
	case '1':
		$(parent).find('#sql-setting').hide();
		$(parent).find('#codelist-setting').hide();
		break;
	case '2':
		$(parent).find('#sql-setting').hide();
		$(parent).find("#accordion").show();
		$(parent).find('#codelist-setting').show();
		
		var count = $(parent).find("input[name=localCodelist]:checked");
		
		if (count == undefined || count.length == 0) {
			$(parent).find("input[name=localCodelist][value=1]").prop("checked", true);
		}
		break;
	}
}
		...
		if (data.physicaldatatype == 5 || data.physicaldatatype == 6 || data.physicaldatatype == 7)  {
			...
			try {
				...
				if (isNaN(valid) && $(obj).val().length > 0) {
					$.qp.showAlertModal(dbMsgSource['sc.screendesign.0279']);
					return;
				}
			} catch(err) {
				$.qp.showAlertModal(dbMsgSource['sc.screendesign.0280']+typeName);
				return;
			}
	...
	case 10:
		...
		if (dataType == 8) {
			try {
				...
				if ($(obj) != undefined && $(obj) != null && $(obj).val().length > 0 && isNaN(valid) && $(obj).autoNumeric('get').length > 0) {
					$.qp.showAlertModal(dbMsgSource['sc.screendesign.0281']);
					$(obj).focus();
					return;
				}
			} catch(err) {
			...
			try {
				...
				if (isNaN(valid) && $(obj).autoNumeric('get').length > 0) {
					$.qp.showAlertModal(dbMsgSource['sc.screendesign.0282']);
					$(obj).focus();
					return;
				}
			} catch(err) {
...
function showDataSourceInView(modal, data) {
	...
	$(modal).find("input:radio[name=localCodelist][value=2]").closest("label").hide();
	...
	if (data.datasourcetype != undefined && data.datasourcetype.length > 0 && data.datasourcetype != 'undefined') {
		...
		$(modal).find("input[name=dataSourceType][value=2]").prop("checked", true);
		changeSettingDataSource($(modal).find('input[name=dataSourceType][value=2]'));
	}
	...
	if(data.datatype != 21 && data.datatype != 22 &&  data.physicaldatatype != undefined && "8" == data.physicaldatatype){
...
function displayParamAction(modal, data) {
	if (data.actiontype != undefined && data.actiontype.length > 0 && data.actiontype != 'undefined') {
		$(modal).find("input[name=actiontype][value="+data.actiontype+"]").prop("checked", true);
		...
		if (data.actiontype == 0 || data.actiontype == '0') {
			...
			if (data.navigateToBlogic != undefined && data.navigateToBlogic != null && data.navigateToBlogic.length > 0) {
				//getModuleByScreenId(data.navigateTo, $(modal));
				getModuleByBlogicId(data.navigateToBlogic, $(modal));
			}
			
			loadParameter($(modal).find("#navigateToScreenBlogic").find("input[name='navigateToBlogic']"), data['parameters']);
			...
			loadParameter($(modal).find("#navigateToScreenBlogic").find("input[name='navigateToBlogicAutocomplete']"), data['parameters']);
		}
		
	} else {
		$(modal).find("input[name=actiontype][value=0]").prop("checked", true);
		setNavigateType(0, modal);
...
function changeModule(event) {
	...
	var blogic = $(event.target).closest("table").find("#navigateToBlogicAutocompleteId");
	var nextHidden = nextInput.next("input[type=hidden]");
	nextInput.val("");
	nextInput.attr("arg01",value);
	/*blogic.attr("arg05",value);*/
	nextHidden.val("");
...
function getModuleByScreenId(screenId, modal) {
	if (screenId == undefined || screenId == null || screenId.length == 0) return;
...
function getModuleByBlogicId(blogicId, modal) {
	if (blogicId == undefined || blogicId == null || blogicId.length == 0) return;
...
function changeScreenId(obj) {
	var screenId = $(obj.target).next("input").val();
	$(obj.target).closest("tr").next().find("input[name='screenItemCodeListAutocomplete']").attr("arg01",screenId);
}

function getCodelistItemByScreenItemId(modal,itemId){
	var url = CONTEXT_PATH + "/screendesign/getItemCodelist?screenItemId="+itemId+"" +"&r="+Math.random();	
	var data = $.qp.getData(url);
	
	var tableOption = $(modal).find("#dialog-component-list-setting-tbl-options");
	$(tableOption).find("tbody").remove();
	/*var dataText = $.qp.getTextResult(url);*/
	if (data != '' && data != null){
		var checkbox = $(tableOption).closest("tr").prev().find("input");
		var content = "<tbody>";
		for(var i = 0; i < data.length; i++){
			if(data[i]["supportOptionValFlg"] == 0) {
				$(checkbox).prop("checked",true);
				$(checkbox).parents("tr").next().find("table th:first").attr("style","width:3%");
			} else {
				$(checkbox).prop("checked",false);
				$(checkbox).parents("tr").next().find("table th:first").attr("style","width:5%");
			}
			var codelistName = data[i]["codelistName"];
			var codelistVal = data[i]["codelistVal"];
			content +="<tr><td>"+(i+1)+"</td><td class=\"colOptionName\">"+codelistName+"</td><td>"+codelistVal+"</td></tr>";
			
		}
		content += "</tbody>";
		tableOption.append(content);
		if($(checkbox).prop('checked') == true){
			$(checkbox).parents("tr").next().find("table").find(".colOptionName").hide();
		} else {
			$(checkbox).parents("tr").next().find("table").find(".colOptionName").show();
		}
		
	}
}

function changeScreenItemId(obj) {
	var screenItemName = "";
	if($(obj.target).next("input").val() != undefined && $(obj.target).next("input").val() != '') {
		screenItemName = $(obj.target).next("input").val();
	}
	var modal =  $(obj.target).closest(".modal");
	var screenId = "";
	if($(obj.target).attr("arg01") != undefined && $(obj.target).attr("arg01") != '') {
		screenId = $(obj.target).attr("arg01");
	}
	getCodelistItemByScreenItemName(modal,screenItemName,screenId);
}

function getCodelistItemByScreenItemName(modal,screenItemName,screenId){
	var url = CONTEXT_PATH + "/screendesign/getItemCodelistByItemName?screenItemName="+screenItemName+"&screenId="+screenId+"" +"&r="+Math.random();	
	var data = $.qp.getData(url);
...
function setValueEnableSort(obj) {
	if($(obj).prop("checked") == true) {
		$(obj).closest("div").find("tr.sql-name").show();
	}
	if($(obj).prop("checked") == false) {
		$(obj).closest("div").find("tr.sql-name").hide();
	}
}

function changeSQLOutput(obj) {
	var sqlId = $(obj.target).next("input").val();
	if(sqlId != "" && sqlId != undefined) {
		var url = CONTEXT_PATH + "/screendesign/getSqlDesign?sqlId="+sqlId+"&r="+Math.random();	
		var data = $.qp.getData(url);
		$(obj.target).attr("arg02",data);
	} else {
		$(obj.target).attr("arg02","");
	}
	
	var table = $(obj.target).closest("tr").next("tr").find("table");
	$(table).find(".qp-input-autocomplete").each(function(){
		$(this).attr("arg02",$(obj.target).attr("arg02"));
		$(this).attr("arg01",$(obj.target).next("input").val());
	});

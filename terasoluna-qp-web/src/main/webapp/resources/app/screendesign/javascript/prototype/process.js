...
function changeSubmitDialogAction(obj){
	...
	if ($(table).find('input[name=actiontype][value=0]').prop('checked', true)) {
		
		$(table).find("#dialog-form-parameter-tbl-parameter").show();
		$(table).find("#blogic-param").hide();
	} else if ($(table).find('input[name=actiontype][value=1]').prop('checked', true)) {
...
function saveDialogCustomSetting() {
	...
	data["customItemContent"] = $(modal).find("textarea[name='customItemContent']").val();
	
	if ($(modal).find("textarea[name='customItemContent']").val() == null || $(modal).find("textarea[name='customItemContent']").val().length == 0)  {
	...
	if (mergeTable(obj, data, rowspan, colspan)) {
		...
		if ((modalName != null && modalName == 'dialog-label-setting-section') || isComponent) {
			$(obj).next().val(value);
		} else {
			$(obj).closest("th").find("input[name='formElement']").val(value);
		}
...
function saveDialogCustomSectionSetting() {
	...
	var obj = $(modal).data('target');
	$(obj).closest("span").find("input[name='formCustomSectionContent']").val($(modal).find("textarea[name='customSectionContent']").val());
...
function selectColumn(data) {
	...
	if (data != null && data.length > 0) {
		...
		var value = $("#srcgenControlElement").find("input[name=radioDataType]:checked").val();
		if (value == 1) {
...
function changeDragElementDataType(element) {
	...
	if ($("input[name=tableColumnMapping]").val() == undefined || $("input[name=tableColumnMapping]").val() == null || $("input[name=tableColumnMapping]").val().length == 0) {
		return;
	}
...
function setNavigateType(value, modal) {
	if (value == 0 || value == '0') {
		...
		$(modal).find('[name=actiontype][value='+value+']').prop('checked', true);
...
function saveTabSetting(obj) {
	...
	if (errorIndex >= 0) {
		alert(dbMsgSource['sc.screendesign.0378']);
				...
				$(modal).find("#tab-design-" + i).find("table tbody tr").each(function() {
					if ($(this).find("input[type=checkbox]").prop("checked")) {
						...
						if (areaHidden != undefined) {
							...
							if ($(area).width() > $(tabContent).width() - 4) {
								$(area).css("width", "100%");
							}
...
function saveIcon(obj, modalName) {
	...
	//get icon
	var icon = $(modal).find('a[selected="selected"]').find('span').attr('class');
...
function buttonTypeChange(obj) {
	...
	if (!$(obj).prop('checked')) {
		...
		if (screenId != undefined && screenId != null) {
			var url = CONTEXT_PATH + "/screendesign/getBlogicInit?screenId="+screenId +"&r="+Math.random();	
			
			var result = $.qp.getString(url);
			var arr = result.split('||'); 
			
			if (arr != null && arr.length > 0) {
				$(obj).closest('table').find('#navigateToBlogicAutocompleteId').val(arr[1]);
				$(obj).closest('table').find('#navigateToBlogic').val(arr[0]);
			}
			$.qp.disableAutocomplete($(obj).closest('table').find("#navigateToScreenBlogic").find("input[name=navigateToBlogic]"));
		}
		
	} else {
		$.qp.enableAutocomplete($(obj).closest('table').find("#navigateToScreenBlogic").find("input[name=navigateToBlogic]"));
	}
}

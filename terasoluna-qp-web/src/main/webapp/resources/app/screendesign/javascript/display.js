...
function openScreenSetting(obj){
	...
	if(parameters != undefined && parameters.length > 0){
		...
		$(arrParameters).each(function(i){
			...
			$(modal).find("#dialog-form-parameter-tbl-parameter").find("tbody").find("tr:eq("+i+")").find("input[name=parameterId]").val(dataParameter[3]);
		});
	} else {
		...
		$(modal).find("#dialog-form-parameter-tbl-parameter").find("tbody").find("tr:eq(0)").find("input[name=parameterId]").val("");
	}
	setFirstTabActive($(modal));
	$(modal).modal({show: true,closable: false,keyboard:false,backdrop:'static'});
}
...
function openTableListSetting(obj){
	...
	var displayPageable = $(obj).closest("span").find("[name=formDisplayPageable]").val();
	$(modal).find("[name=displayPageable]").val(displayPageable);
	if(displayPageable == 1) {
		$(modal).find("[name=displayPageable]").prop("checked",true);
	} else {
		$(modal).find("[name=displayPageable]").prop("checked",false);
	}
	...
	if ($(obj).closest("span").find("[name=formFixedRow]").val() != undefined && $(obj).closest("span").find("[name=formFixedRow]").val() != null && ($(obj).closest("span").find("[name=formFixedRow]").val() == 1 || $(obj).closest("span").find("[name=formFixedRow]").val() == "1")) {
		$(modal).find("[name=fixedRow]").val($(obj).closest("span").find("[name=formFixedRow]").val());
	...
	if ($(obj).closest("span").find("[name=formAreaTypeAction]").val() != undefined && $(obj).closest("span").find("[name=formAreaTypeAction]").val() != null 
		...
		if ($(obj).closest("span").find("[name=formAreaTypeAction]").val() == 0 || $(obj).closest("span").find("[name=formAreaTypeAction]").val() == 2) {
			$(modal).find("[name=areaTypeActionField][value="+$(obj).closest("span").find("[name=formAreaTypeAction]").val()+"]").prop("checked", true);
	...
	var projectId = $(obj).closest("form").find("input[name='projectId']").val();
	var moduleId = $(obj).closest("form").find("input[name='moduleId']").val();
	$(modal).find("#modal-table-list-setting-header-sort").find("input[id='sqlnameAutocompleteId']").attr("arg03",projectId);
	$(modal).find("#modal-table-list-setting-header-sort").find("input[id='sqlnameAutocompleteId']").attr("arg04",moduleId);
	
	$(obj).closest("div").next("div").find("th").each(function(){
	...
	if(screenPatternType == 1) {
		openHeaderSort(modal, obj);
	} else {
		$(modal).find("a[href='#modal-table-list-setting-header-sort']").closest("li").hide();
	}
	setFirstTabActive($(modal));
	$(modal).modal({show: true,closable: false,keyboard:false,backdrop:'static'});
}
...
function openActionAreaSetting(obj){
	...
	if(parameters != undefined && parameters.length > 0){
		...
		$(arrParameters).each(function(i){
			...
			$(modal).find("#tbl-hiddenAttibutes").find("tbody").find("tr:last").find("input[name=labelName]").val(dataParameter[1]);
			$(modal).find("#tbl-hiddenAttibutes").find("tbody").find("tr:last").find("input[name=labelNameAutocomplete]").val(dataParameter[2]);
			if(dataParameter[4] != undefined && dataParameter[4] != "") {
				$(modal).find("#tbl-hiddenAttibutes").find("tbody").find("tr:last").find("select[name=parameterDatatype] option[value="+dataParameter[4]+"]").prop("selected",true);
			} else {
				$(modal).find("#tbl-hiddenAttibutes").find("tbody").find("tr:last").find("select[name=parameterDatatype] option[value="+dataParameter[3]+"]").prop("selected",true);
			}
...
//		$("#settingDefaultValue").find("input[name=defaultValue]").prop("checked", false);
		$("#settingDefaultValue").find("input[name=defaultValue][value=5]").prop("checked", true);
	}
	...
	if (datatype == '9') {
		...
		$("#settingDefaultValue").find("input[name=defaultValue][value=5]").closest("label").show();
	} else {
		$("#settingDefaultValue").find("input[name=defaultValue][value=4]").closest("label").hide();
		$("#settingDefaultValue").find("input[name=defaultValue][value!=4]").closest("label").show();
	}
